...
StatementType IfStatement::getStatementType() { return StatementType::IF; }

StatementType CallStatement::getStatementType() { return StatementType::CALL; }

StatementType AssignStatement::getStatementType() {
  return StatementType::ASSIGN;
}

StatementType ReadStatement::getStatementType() { return StatementType::READ; }

string Constant::format(int _) { return value; }
...
string CombinationExpression::format(int level) {
  ...
  return "(" + lhs->format(level) + " " + getBopLabel(op) + " " +
         rhs->format(level) + ")";
}

string BooleanExpression::format(int level) {
  return "(" + lhs->format(level) +
         (op == BooleanOperator::AND ? " && " : " || ") + rhs->format(level) +
         ")";
}
...
string RelationalExpression::format(int level) {
  return "(" + lhs->format(level) + " " + getRopLabel(op) + " " +
         rhs->format(level) + ")";
}
...
string Identifier::format(int _) { return "$" + name; }

string ErrorStatement::format(int _) { return "ERROR;\n"; }
...
string IfStatement::format(int level) {
  ...
  return header + "if (" + cond->format(level) + ") then {\n" +
         consequent->format(level + 1) + header + "} else {\n" +
         alternative->format(level + 1) + header + "}\n";
}

string AssignStatement::format(int level) {
  return Statement::format(level) + id->format(level) + " = " +
         expr->format(level) + ";\n";
}

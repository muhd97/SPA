...
public:
  ...
  const string &getEntityTypeName() const { return entityTypeName; }
...
public:
  ...
  Declaration(shared_ptr<DesignEntity> ent, vector<shared_ptr<Synonym>> vec)
      : synonyms(move(vec)), de(move(ent)) {}

  const vector<shared_ptr<Synonym>> &getSynonyms() const { return synonyms; }

  shared_ptr<DesignEntity> getDesignEntity() { return de; }
...
class StmtRef {
public:
  StmtRef(StmtRefType type) { stmtRefType = type; }
  ...
  StmtRefType getStmtRefType() { return stmtRefType; }
  ...
  const string &getStringVal() const { return stringValue; }

  int getIntVal() { return intValue; }
...
public:
  EntRef(EntRefType type) { entRefType = type; }
  ...
  EntRefType getEntRefType() { return entRefType; }
  ...
  string format() { return getEntRefTypeName(); }
};
...
class RelRef {
public:
  virtual string format() { return "RelRef THIS SHOULD NOT BE PRINTED"; }
...
public:
  ...
  UsesS(shared_ptr<StmtRef> sRef, shared_ptr<EntRef> eRef)
      : stmtRef(move(sRef)), entRef(move(eRef)) {}

  string format() override {
    return "UsesS(" + stmtRef->getStmtRefTypeName() + ", " +
           entRef->getEntRefTypeName() + ")";
  }
  ...
  inline RelRefType getType() { return RelRefType::USES_S; }
...
public:
  ...
  UsesP(shared_ptr<EntRef> eRef1, shared_ptr<EntRef> eRef2)
      : entRef1(move(eRef1)), entRef2(move(eRef2)) {}

  string format() override {
    return "UsesP(" + entRef1->getEntRefTypeName() + ", " +
           entRef2->getEntRefTypeName() + ")";
  }
  ...
  inline RelRefType getType() { return RelRefType::USES_P; }
...
public:
  ...
  ModifiesS(shared_ptr<StmtRef> sRef, shared_ptr<EntRef> eRef)
      : stmtRef(move(sRef)), entRef(move(eRef)) {}

  string format() override {
    return "ModifiesS(" + stmtRef->getStmtRefTypeName() + ", " +
           entRef->getEntRefTypeName() + ")";
  }
  ...
  inline RelRefType getType() { return RelRefType::MODIFIES_S; }
...
public:
  ...
  ModifiesP(shared_ptr<EntRef> eRef1, shared_ptr<EntRef> eRef2)
      : entRef1(move(eRef1)), entRef2(move(eRef2)) {}
  ...
  inline RelRefType getType() { return RelRefType::MODIFIES_P; }

  string format() override {
    return "ModifiesP(" + entRef1->getEntRefTypeName() + ", " +
           entRef2->getEntRefTypeName() + ")";
  }
...
public:
  ...
  Parent(shared_ptr<StmtRef> sRef1, shared_ptr<StmtRef> sRef2)
      : stmtRef1(move(sRef1)), stmtRef2(move(sRef2)) {}

  string format() override {
    return "Parent(" + stmtRef1->getStmtRefTypeName() + ", " +
           stmtRef2->getStmtRefTypeName() + ")";
  }
  ...
  inline RelRefType getType() { return RelRefType::PARENT; }
...
public:
  ...
  ParentT(shared_ptr<StmtRef> sRef1, shared_ptr<StmtRef> sRef2)
      : stmtRef1(move(sRef1)), stmtRef2(move(sRef2)) {}
  ...
  inline RelRefType getType() { return RelRefType::PARENT_T; }
...
public:
  ...
  Follows(shared_ptr<StmtRef> sRef1, shared_ptr<StmtRef> sRef2)
      : stmtRef1(move(sRef1)), stmtRef2(move(sRef2)) {}

  string format() override {
    return "Follows(" + stmtRef1->getStmtRefTypeName() + ", " +
           stmtRef2->getStmtRefTypeName() + ")";
  }
  ...
  inline RelRefType getType() { return RelRefType::FOLLOWS; }
...
public:
  ...
  FollowsT(shared_ptr<StmtRef> sRef1, shared_ptr<StmtRef> sRef2)
      : stmtRef1(move(sRef1)), stmtRef2(move(sRef2)) {}
  ...
  inline RelRefType getType() { return RelRefType::FOLLOWS_T; }
...
public:
  ...
  ExpressionSpec(bool isAnything, bool isPartialMatch,
                 shared_ptr<Expression> expression)
...
public:
  ...
  PatternCl(shared_ptr<Synonym> synonym, shared_ptr<EntRef> entRef,
            shared_ptr<ExpressionSpec> expression)
      : synonym(move(synonym)), entRef(move(entRef)),
        exprSpec(move(expression)) {}
  ...
  string format() {
    return "\nPATTERN " + synonym->format() + " (" + entRef->format() + ", " +
           exprSpec->format() + ")";
  }
...
public:
  ...
  SelectCl(shared_ptr<Synonym> syn, vector<shared_ptr<Declaration>> decl,
           vector<shared_ptr<SuchThatCl>> stht,
           vector<shared_ptr<PatternCl>> pttn)
      : targetSynonym(move(syn)), declarations(move(decl)),
        suchThatClauses(move(stht)), patternClauses(move(pttn)) {

    for (auto &d : declarations) {
      for (auto syn : d->synonyms) {

        /* Duplicate declaration is detected. This synonym was previously
         * already encountered in a declaration, but is now encountered again!
         */
        if (synonymToParentDeclarationMap.find(syn->getValue()) !=
            synonymToParentDeclarationMap.end()) {
          throw std::invalid_argument(
              "Error: Duplicate synonym detected in query!");
        }
  ...
  getParentDeclarationForSynonym(shared_ptr<Synonym> s) {
    if (synonymToParentDeclarationMap.find(s->getValue()) ==
        synonymToParentDeclarationMap.end()) {
  ...
  inline bool isSynonymDeclared(string toTest) {
    return synonymToParentDeclarationMap.find(toTest) !=
           synonymToParentDeclarationMap.end();
  }

  inline string getDesignEntityTypeBySynonym(string s) {
    if (synonymToParentDeclarationMap.find(s) ==
        synonymToParentDeclarationMap.end()) {
  ...
  inline bool hasSuchThatClauses() { return suchThatClauses.size() > 0; }

  inline bool hasPatternClauses() { return patternClauses.size() > 0; }

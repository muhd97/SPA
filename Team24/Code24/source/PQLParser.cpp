...
PQLToken PQLParser::peek() { return tokens[index]; }

PQLToken PQLParser::peekNext() { return tokens[index + 1]; }
...
bool PQLParser::tokensAreEmpty() { return index == size; }
...
inline PQLToken PQLParser::eat(PQLTokenType exepctedType) {
  ...
  if (tok.type == exepctedType) {
    ...
    cout << "Expected: " << getPQLTokenLabel(temp)
         << " but got: " << getPQLTokenLabel(tok) << " instead\n";
...
inline PQLToken PQLParser::eatKeyword(string keyword) {
  ...
  if (tok.type == PQLTokenType::NAME && tok.stringValue == keyword) {
    ...
    cout << "Expected: " << keyword << " but got: " << getPQLTokenLabel(tok)
         << " instead\n";
    throw std::invalid_argument("Error parsing PQL Query!!");
  }
  return tok;
...
shared_ptr<DesignEntity> PQLParser::parseDesignEntity() {
  ...
  if (peek().type == PQLTokenType::NAME) {
    ...
    cout << "Expected stmt, read, print... but got: " << getPQLTokenLabel(curr)
         << " instead\n";
    throw std::invalid_argument("Error parsing PQL Query!!");
  }
}
...
shared_ptr<StmtRef> PQLParser::parseStmtRef() {
  ...
  default:
    return make_shared<StmtRef>(StmtRefType::SYNONYM,
                                parseSynonym()->getValue());
  }
}
...
shared_ptr<RelRef> PQLParser::parseUses() {
  ...
  if (peek().type == PQLTokenType::STRING) { /* If first arg of Uses() is a
...
shared_ptr<RelRef> PQLParser::parseModifies() {
  ...
  eat(PQLTokenType::LEFT_PAREN);
  if (peek().type !=
      PQLTokenType::STRING) { /* If first arg of Modifies() is a string, it must
                                 be a ModifiesP */
...
inline bool tokenIsDesignEntity(PQLToken tk) {
  return tk.type == PQLTokenType::NAME &&
         (tk.stringValue == PQL_STMT || tk.stringValue == PQL_READ ||
          tk.stringValue == PQL_PRINT || tk.stringValue == PQL_CALL ||
          tk.stringValue == PQL_WHILE || tk.stringValue == PQL_IF ||
          tk.stringValue == PQL_ASSIGN || tk.stringValue == PQL_VARIABLE ||
          tk.stringValue == PQL_CONSTANT || tk.stringValue == PQL_PROCEDURE);
}
...
shared_ptr<SelectCl> PQLParser::parseSelectCl() {
  ...
  while (!tokensAreEmpty()) {
    ...
    if (peek().type == PQLTokenType::NAME &&
        peek().stringValue ==
            PQL_SUCH) { /* YIDA: Comment this to DISABLE Multiple Clauses */
    ...
    } else if (peek().type == PQLTokenType::NAME &&
               peek().stringValue == PQL_PATTERN) {
      ...
      cout << "ParseSelectCl Unknown token: " + getPQLTokenLabel(peek())
           << endl;
      break;
    }
  }

  return make_shared<SelectCl>(move(synonym), move(declarations),
                               move(suchThatClauses), move(patternClauses));
}

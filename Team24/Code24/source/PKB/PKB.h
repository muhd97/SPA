...
public:
  ...
  unordered_map<PKBDesignEntity, set<PKBVariable::SharedPtr>>
      mModifiedVariables;
  ...
  unordered_map<PKBDesignEntity, set<PKBStatement::SharedPtr>>
      designEntityToStatementsThatUseVarsMap;

  set<PKBStatement::SharedPtr> setOfProceduresThatUseVars;
  unordered_map<string, set<PKBStatement::SharedPtr>>
      variableNameToProceduresThatUseVarMap;

  set<PKBStatement::SharedPtr>
      mAllModifyStmts; // statements that modify a variable
  unordered_map<PKBDesignEntity, set<PKBStatement::SharedPtr>>
      designEntityToStatementsThatModifyVarsMap;

  set<PKBStatement::SharedPtr>
      mProceduresThatModifyVars; // procedures that modify at least one variable
  unordered_map<string, set<PKBStatement::SharedPtr>>
      mVariableNameToProceduresThatModifyVarsMap;
  ...
  bool getStatement(int stmtNumber, PKBStatement::SharedPtr &stmt) {
    if (stmtNumber < 1 ||
        stmtNumber >
            (int)mStatements[PKBDesignEntity::AllExceptProcedure].size()) {
    ...
    stmt = mStatements[PKBDesignEntity::AllExceptProcedure]
                      [targetIndexInMStatementsVector];
  ...
  int getUsedVariablesSize() { return mUsedVariables.size(); }
  ...
  PKBVariable::SharedPtr getVarByName(string s) {
    if (mVariables.find(s) ==
        mVariables
            .end()) { /* Exceptional Case: requested variable is NOT found. */
      return nullptr;
    }
    return mVariables[s];
  }

  set<PKBStatement::SharedPtr> getAllUseStmts() { return mAllUseStmts; }
  ...
  PKBStatement::SharedPtr getProcedureByName(string procname) {
    if (procedureNameToProcedureMap.find(procname) ==
        procedureNameToProcedureMap.end()) {
      return nullptr;
    }
  ...
  unordered_set<string> getConstants() { return mConstants; }

  bool getCached(Relation rel, PKBDesignEntity a, PKBDesignEntity b,
                 vector<int> &res) {
    try {
  ...
  bool getCachedSet(Relation rel, PKBDesignEntity a, PKBDesignEntity b,
  ...
  void insertintoCache(Relation rel, PKBDesignEntity a, PKBDesignEntity b,
                       vector<int> &res) {
    cache[rel][a][b] = res;
  }

  void insertintoCacheSet(Relation rel, PKBDesignEntity a, PKBDesignEntity b,
                          set<pair<int, int>> &res) {
    cacheSet[rel][a][b] = res;
  }

  const unordered_map<string, PKBVariable::SharedPtr> &
  getAllVariablesMap() const;
...
protected:
  ...
  map<Relation, map<PKBDesignEntity, map<PKBDesignEntity, set<pair<int, int>>>>>
      cacheSet;

  void addStatement(PKBStatement::SharedPtr &statement,
                    PKBDesignEntity designEntity);
  inline void addUsedVariable(PKBDesignEntity designEntity,
                              PKBVariable::SharedPtr &variable);
  void addUsedVariable(PKBDesignEntity designEntity,
                       set<PKBVariable::SharedPtr> &variables);
  inline void addModifiedVariable(PKBDesignEntity designEntity,
                                  PKBVariable::SharedPtr &variable);
  void addModifiedVariable(PKBDesignEntity designEntity,
                           set<PKBVariable::SharedPtr> &variables);

  PKBVariable::SharedPtr getVariable(string name);

  PKBStatement::SharedPtr extractProcedure(shared_ptr<Procedure> &procedure);
  PKBStatement::SharedPtr extractStatement(shared_ptr<Statement> &statement,
                                           PKBGroup::SharedPtr &parentGroup);

  PKBStatement::SharedPtr
  extractAssignStatement(shared_ptr<Statement> &statement,
                         PKBGroup::SharedPtr &parentGroup);
  PKBStatement::SharedPtr
  extractReadStatement(shared_ptr<Statement> &statement,
                       PKBGroup::SharedPtr &parentGroup);
  PKBStatement::SharedPtr
  extractPrintStatement(shared_ptr<Statement> &statement,
                        PKBGroup::SharedPtr &parentGroup);
  PKBStatement::SharedPtr extractIfStatement(shared_ptr<Statement> &statement,
                                             PKBGroup::SharedPtr &parentGroup);
  PKBStatement::SharedPtr
  extractWhileStatement(shared_ptr<Statement> &statement,
                        PKBGroup::SharedPtr &parentGroup);
  PKBStatement::SharedPtr
  extractCallStatement(shared_ptr<Statement> &statement,
                       PKBGroup::SharedPtr &parentGroup);

  PKBStatement::SharedPtr createPKBStatement(shared_ptr<Statement> &statement,
                                             PKBGroup::SharedPtr &parentGroup);
  PKBGroup::SharedPtr createPKBGroup(string &name,
                                     PKBStatement::SharedPtr &ownerStatement);
  PKBGroup::SharedPtr createPKBGroup(PKBStatement::SharedPtr &ownerStatement,
                                     PKBGroup::SharedPtr &parentGroup);

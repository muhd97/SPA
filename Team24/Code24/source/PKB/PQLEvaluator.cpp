...
set<pair<int, int>> PQLEvaluator::getParents(PKBDesignEntity parentType,
                                             PKBDesignEntity childType) {
...
bool PQLEvaluator::getParentsUnderscoreUnderscore() {
  ...
  for (auto &stmt : parentStmts) {
    ...
    for (auto &grp : grps) {
      vector<int> members =
          grp->getMembers(PKBDesignEntity::AllExceptProcedure);
      if (!members.empty())
        return true;
    }
  }
...
set<pair<int, int>> PQLEvaluator::getParentsT(PKBDesignEntity parentType,
                                              PKBDesignEntity childType) {
  set<pair<int, int>> res;
  // vector<int> temp;
...
bool PQLEvaluator::hasEligibleChildRecursive(PKBGroup::SharedPtr grp,
                                             PKBDesignEntity parentType,
                                             PKBDesignEntity childType,
...
set<int> PQLEvaluator::getChildrenT(PKBDesignEntity childType,
                                    int parentIndex) {
  set<int> res;
  // PKBStatement::SharedPtr parent;
...
unordered_set<int> PQLEvaluator::getAllChildAndSubChildrenOfGivenType(
    PKBStatement::SharedPtr targetParent, PKBDesignEntity targetChildrenType) {
...
unordered_set<int>
PQLEvaluator::getParentTIntSyn(int statementNo,
...
bool PQLEvaluator::getParentTIntInt(int parentStatementNo,
                                    int childStatementNo) {
  unordered_set<int> toReturn;
  queue<PKBGroup::SharedPtr> qOfGroups;
  PKBStatement::SharedPtr stmt;
  if (!mpPKB->getStatement(parentStatementNo, stmt)) {
...
unordered_set<int>
PQLEvaluator::getParentTSynUnderscore(PKBDesignEntity targetParentType) {
  unordered_set<int> toReturn;
  vector<PKBStatement::SharedPtr> parentStmts;
...
unordered_set<int>
PQLEvaluator::getParentTSynInt(PKBDesignEntity targetParentType,
                               int childStatementNo) {
  unordered_set<int> toReturn;
  vector<PKBStatement::SharedPtr> parentStmts;
...
set<pair<int, int>> PQLEvaluator::getParentTSynSyn(PKBDesignEntity parentType,
                                                   PKBDesignEntity childType) {
  set<pair<int, int>> res;
...
unordered_set<int>
PQLEvaluator::getParentTUnderscoreSyn(PKBDesignEntity targetChildType) {
  unordered_set<int> toReturn;
  ...
  for (const auto &stmt : parentStmts) {
    for (const int &i :
         getAllChildAndSubChildrenOfGivenType(stmt, targetChildType)) {
      toReturn.insert(i);
    }
  }
...
vector<int> PQLEvaluator::getBefore(PKBDesignEntity beforeType,
                                    int afterIndex) {
  vector<int> res;
  PKBStatement::SharedPtr stmt;
  ...
  if (beforeType == PKBDesignEntity::AllExceptProcedure ||
      stmtBefore->getType() == beforeType) {
...
bool PQLEvaluator::getStatementBefore(PKBStatement::SharedPtr &statementAfter,
                                      PKBStatement::SharedPtr &result) {
...
bool PQLEvaluator::getStatementAfter(PKBStatement::SharedPtr &statementBefore,
                                     PKBStatement::SharedPtr &result) {
  // find the rightStatement before in the stmt's group
 ...
 * after after */
set<pair<int, int>> PQLEvaluator::getBeforePairs(PKBDesignEntity beforeType,
                                                 PKBDesignEntity afterType) {
  set<pair<int, int>> res;
  // check if res is cached, if so return results
...
vector<int> PQLEvaluator::getAfter(PKBDesignEntity afterType, int beforeIndex) {
  ...
  // if pass the type check
  if (afterType == PKBDesignEntity::AllExceptProcedure ||
      stmtAfter->getType() == afterType) {
    // and pass the same nesting level check
...
bool PQLEvaluator::getFollowsUnderscoreUnderscore() {
  vector<PKBStatement::SharedPtr> stmts =
      mpPKB->getStatements(PKBDesignEntity::AllExceptProcedure);
  PKBStatement::SharedPtr stmtAfter;
  for (auto &stmt : stmts) {
    // if there is no rightStatement after, go next
    if (!getStatementAfter(stmt, stmtAfter)) {
...
bool PQLEvaluator::getFollowsTIntegerInteger(int leftStmtNo, int rightStmtNo) {
  ...
  if (leftStatement->getGroup() == rightStatement->getGroup() &&
      leftStmtNo < rightStmtNo) {
    return true;
  }
  return false;
}
...
// getAfterT
unordered_set<int>
PQLEvaluator::getFollowsTIntegerSyn(PKBDesignEntity rightType, int leftStmtNo) {
  ...
  for (auto &rightStmtNo = grpMembers.rbegin();
       rightStmtNo != grpMembers.rend(); ++rightStmtNo) {
...
bool PQLEvaluator::getFollowsTIntegerUnderscore(int leftStmtNo) {
  ...
  vector<int> members = leftStatement->getGroup()->getMembers(
      PKBDesignEntity::AllExceptProcedure);
  return leftStmtNo < members.back();
}

// getBeforeT
unordered_set<int> PQLEvaluator::getFollowsTSynInteger(PKBDesignEntity leftType,
                                                       int rightStmtNo) {
  unordered_set<int> toReturn;
  PKBStatement::SharedPtr rightStatement;
...
set<pair<int, int>> PQLEvaluator::getFollowsTSynSyn(PKBDesignEntity leftType,
                                                    PKBDesignEntity rightType) {
  set<pair<int, int>> toReturn;
  // get results manually
  // get all the 'before' users first
  vector<PKBStatement::SharedPtr> beforeStatements =
      mpPKB->getStatements(leftType);
  ...
  for (int i = beforeStatements.size() - 1; i >= 0; i--) {
    ...
    for (int j = afterStatements.size() - 1; j >= 0;
         j--) { // count from back again
...
unordered_set<int>
PQLEvaluator::getFollowsTSynUnderscore(PKBDesignEntity leftType) {
  unordered_set<int> toReturn;
...
bool PQLEvaluator::getFollowsTUnderscoreInteger(int rightStmtNo) {
  ...
  vector<int> members = rightStatement->getGroup()->getMembers(
      PKBDesignEntity::AllExceptProcedure);
...
unordered_set<int>
PQLEvaluator::getFollowsTUnderscoreSyn(PKBDesignEntity rightType) {
  unordered_set<int> toReturn;

  // get results manually
  // get all the 'after' users first
  vector<PKBStatement::SharedPtr> rightStatements =
      mpPKB->getStatements(rightType);

  // count from the back, using rbegin and rend
  for (int i = rightStatements.size() - 1; i >= 0; i--) {
    auto &currStmt = rightStatements[i];
    PKBGroup::SharedPtr grp = currStmt->getGroup();
    vector<int> leftStatements =
        grp->getMembers(PKBDesignEntity::AllExceptProcedure);
...
bool PQLEvaluator::getFollowsTUnderscoreUnderscore() {
  vector<PKBStatement::SharedPtr> allStatements =
      mpPKB->getStatements(PKBDesignEntity::AllExceptProcedure);
...
vector<string> PQLEvaluator::getUsed() {
  set<PKBVariable::SharedPtr> vars =
      mpPKB->getUsedVariables(PKBDesignEntity::AllExceptProcedure);
  return varToString(move(vars));
}
...
vector<int> PQLEvaluator::getUsers(PKBDesignEntity userType,
                                   string variableName) {
...
vector<int> PQLEvaluator::getUsers(PKBDesignEntity entityType) {
  ...
  set<PKBStatement::SharedPtr> &useStmtsToCopyOver =
      entityType != PKBDesignEntity::AllExceptProcedure
          ? mpPKB->getAllUseStmts(entityType)
          : mpPKB->getAllUseStmts();
...
vector<string> PQLEvaluator::getProceduresThatUseVar(string variableName) {
  ...
  set<PKBStatement::SharedPtr> &procedures =
      mpPKB->variableNameToProceduresThatUseVarMap[variableName];
...
bool PQLEvaluator::checkModified(PKBDesignEntity entityType, string ident) {
  ...
  if ((targetVar = mpPKB->getVarByName(ident)) == nullptr)
    return false;
  set<PKBVariable::SharedPtr> &allVars =
      mpPKB->getModifiedVariables(entityType);
  return allVars.find(targetVar) != allVars.end();
}

bool PQLEvaluator::checkModified() {
  set<PKBVariable::SharedPtr> &vars =
      mpPKB->getModifiedVariables(PKBDesignEntity::AllExceptProcedure);
  return vars.size() > 0;
}
...
bool PQLEvaluator::checkModifiedByProcName(string procname, string ident) {
  ...
  const set<PKBVariable::SharedPtr> &varsUsed =
      procedure->getModifiedVariables();
...
bool PQLEvaluator::checkAnyProceduresModifyVar(string variableName) {
  ...
  return mpPKB->mVariableNameToProceduresThatModifyVarsMap[variableName]
             .size() > 0;
}
...
vector<string> PQLEvaluator::getModifiedByProcName(string procname) {
  ...
  vector<PKBVariable::SharedPtr> vars;
  const set<PKBVariable::SharedPtr> &varsModified =
      procedure->getModifiedVariables();
...
vector<string> PQLEvaluator::getProceduresThatModifyVar(string variableName) {
  ...
  set<PKBStatement::SharedPtr> &procedures =
      mpPKB->mVariableNameToProceduresThatModifyVarsMap[variableName];
  toReturn.reserve(procedures.size());
...
vector<int> PQLEvaluator::getModifiers(PKBDesignEntity modifierType,
                                       string variableName) {
...
const vector<PKBStatement::SharedPtr> &
PQLEvaluator::getStatementsByPKBDesignEntity(PKBDesignEntity pkbDe) const {
...
vector<PKBVariable::SharedPtr> PQLEvaluator::getAllVariables() {
  const unordered_map<string, PKBVariable::SharedPtr> &map =
      mpPKB->getAllVariablesMap();
...
vector<pair<int, string>> PQLEvaluator::matchAnyPattern(string &LHS) {
  vector<PKBStatement::SharedPtr> assignStmts =
      mpPKB->getStatements(PKBDesignEntity::Assign);
  vector<pair<int, string>> res;
  for (auto &assignStmt : assignStmts) {
    // check LHS
    if (LHS == assignStmt->simpleAssignStatement->getId()->getName() ||
        LHS == "_") {
      int statementIndex = assignStmt->getIndex();
      string variableModified =
          assignStmt->simpleAssignStatement->getId()->getName();
...
PQLEvaluator::matchPartialPattern(string &LHS, shared_ptr<Expression> &RHS) {
  vector<PKBStatement::SharedPtr> assignStmts =
      mpPKB->getStatements(PKBDesignEntity::Assign);
  vector<pair<int, string>> res;
  ...
  for (auto &assignStmt : assignStmts) {
    ...
    // check RHS
    vector<string> assignInOrder =
        inOrderTraversalHelper(assignStmt->simpleAssignStatement->getExpr());
    vector<string> assignPreOrder =
        preOrderTraversalHelper(assignStmt->simpleAssignStatement->getExpr());
    if (checkForSubTree(queryInOrder, assignInOrder) &&
        checkForSubTree(queryPreOrder, assignPreOrder)) {
      int statementIndex = assignStmt->getIndex();
      string variableModified =
          assignStmt->simpleAssignStatement->getId()->getName();
      res.emplace_back(make_pair(statementIndex, variableModified));
...
// instead
vector<pair<int, string>>
PQLEvaluator::matchExactPattern(string &LHS, shared_ptr<Expression> &RHS) {
  vector<PKBStatement::SharedPtr> assignStmts =
      mpPKB->getStatements(PKBDesignEntity::Assign);
  vector<pair<int, string>> res;
  ...
  for (auto &assignStmt : assignStmts) {
    ...
    vector<string> assignPreOrder =
        preOrderTraversalHelper(assignStmt->simpleAssignStatement->getExpr());
    if (checkForExactTree(queryInOrder, assignInOrder) &&
        checkForExactTree(queryPreOrder, assignPreOrder)) {
      int statementIndex = assignStmt->getIndex();
      string variableModified =
          assignStmt->simpleAssignStatement->getId()->getName();
      res.emplace_back(make_pair(statementIndex, variableModified));
...
vector<string>
PQLEvaluator::inOrderTraversalHelper(shared_ptr<Expression> expr) {
  ...
  while (!queue.empty()) {
    ...
    case ExpressionType::COMBINATION: {
      shared_ptr<CombinationExpression> cmb =
          static_pointer_cast<CombinationExpression>(e);
...
vector<string>
PQLEvaluator::preOrderTraversalHelper(shared_ptr<Expression> expr) {
  vector<string> res; // using a set to prevent duplicates
  vector<shared_ptr<Expression>> queue = {expr};
...
bool PQLEvaluator::checkForSubTree(vector<string> &queryInOrder,
                                   vector<string> &assignInOrder) {
  for (size_t assignPointer = 0; assignPointer < assignInOrder.size();
       assignPointer++) {
    // early termination, assignInOrder too small already
    if (assignInOrder.size() - assignPointer < queryInOrder.size()) {
      return false;
    }
    for (size_t queryPointer = 0; queryPointer < queryInOrder.size();
         queryPointer++) {
      if (queryInOrder[queryPointer] !=
          assignInOrder[assignPointer + queryPointer]) {
        break;
      }
...
bool PQLEvaluator::checkForExactTree(vector<string> &queryInOrder,
                                     vector<string> &assignInOrder) {

...
class PKBPQLParentHandler {
public:
    using SharedPtr = std::shared_ptr<PKBPQLParentHandler>;

    static SharedPtr create(PKB::SharedPtr pkb)
    {
        return SharedPtr(new PKBPQLParentHandler(pkb));
    }
...
private:
    ...
    PKBPQLParentHandler(PKB::SharedPtr pkb) {
        mpPKB = pkb;
    };

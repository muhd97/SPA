...
void PKB::extractDesigns(shared_ptr<Program> program) {
  ...
  for (auto &vec : mStatements) {
    std::sort(vec.second.begin(), vec.second.end(),
              [](const PKBStatement::SharedPtr &a,
                 const PKBStatement::SharedPtr &b) -> bool {
                return a->getIndex() < b->getIndex();
              });
  }
}

PKBStatement::SharedPtr PKB::extractStatement(shared_ptr<Statement> &statement,
                                              PKBGroup::SharedPtr &group) {
...
// return PKBStatement of the procedureSimple extracted. Procedure is not
// actually a statement but we represent it with a PKBStatement of type
// PKBDesignEntity::Procedure for now...
PKBStatement::SharedPtr
PKB::extractProcedure(shared_ptr<Procedure> &procedureSimple) {
  // create procedureSimple statement
  PKBStatement::SharedPtr res = PKBStatement::create(
      procedureSimple->getName(), PKBDesignEntity::Procedure);
  ...
  vector<shared_ptr<Statement>> simpleStatements =
      procedureSimple->getStatementList()->getStatements();
  ...
  for (auto &ptr : varsUsedByGroup) {
    if (variableNameToProceduresThatUseVarMap.find(ptr->getName()) ==
        variableNameToProceduresThatUseVarMap.end()) {
  ...
  set<PKBVariable::SharedPtr> varsModifiedByGroup =
      group->getModifiedVariables();
...
PKBStatement::SharedPtr
PKB::extractAssignStatement(shared_ptr<Statement> &statement,
  ...
  shared_ptr<AssignStatement> assignStatement =
      static_pointer_cast<AssignStatement>(statement);
  ...
  designEntityToStatementsThatModifyVarsMap[PKBDesignEntity::Assign].insert(
      res);
...
PKBStatement::SharedPtr
PKB::extractReadStatement(shared_ptr<Statement> &statement,
                          PKBGroup::SharedPtr &parentGroup) {
  // 1. PARENT/FOLLOW - create the PKBStatement, createPKBStatement() handles
  // PARENTS and FOLLOWS
  PKBStatement::SharedPtr res = createPKBStatement(statement, parentGroup);
  shared_ptr<ReadStatement> readStatement =
      static_pointer_cast<ReadStatement>(statement);
...
PKBStatement::SharedPtr
PKB::extractPrintStatement(shared_ptr<Statement> &statement,
                           PKBGroup::SharedPtr &parentGroup) {
  // create the PKBStatement
  PKBStatement::SharedPtr res = createPKBStatement(statement, parentGroup);
  shared_ptr<PrintStatement> printStatement =
      static_pointer_cast<PrintStatement>(statement);
...
PKBStatement::SharedPtr
PKB::extractIfStatement(shared_ptr<Statement> &statement,
                        PKBGroup::SharedPtr &parentGroup) {
  // 1. create a PKBStatement
  PKBStatement::SharedPtr res = createPKBStatement(statement, parentGroup);
  shared_ptr<IfStatement> ifStatement =
      static_pointer_cast<IfStatement>(statement);
  ...
  vector<shared_ptr<Statement>> consequentStatements =
      ifStatement->getConsequent()->getStatements();
  vector<shared_ptr<Statement>> alternativeStatements =
      ifStatement->getAlternative()->getStatements();
  ...
  set<PKBVariable::SharedPtr> &consequentGroupVars =
      consequentGroup->getUsedVariables();
  set<PKBVariable::SharedPtr> &altGroupVars =
      alternativeGroup->getUsedVariables();

  // now the original statement inherits from the group
  res->addUsedVariables(consequentGroupVars);
  res->addUsedVariables(altGroupVars);

  if (identifiers.size() > 0 || consequentGroupVars.size() > 0 ||
      altGroupVars.size() > 0) {
    mAllUseStmts.insert(res);
  ...
  addModifiedVariable(PKBDesignEntity::If,
                      consequentGroup->getModifiedVariables());
  addModifiedVariable(PKBDesignEntity::If,
                      alternativeGroup->getModifiedVariables());
  ...
  if (consequentGroup->getModifiedVariables().size() > 0 ||
      alternativeGroup->getModifiedVariables().size() > 0) {
...
PKBStatement::SharedPtr
PKB::extractWhileStatement(shared_ptr<Statement> &statement,
                           PKBGroup::SharedPtr &parentGroup) {
  // 1. create a PKBStatement
  PKBStatement::SharedPtr res = createPKBStatement(statement, parentGroup);
  shared_ptr<WhileStatement> whileStatement =
      static_pointer_cast<WhileStatement>(statement);
  ...
  if (group->getModifiedVariables().size() > 0) {
    // contained statements of the while loop modify variable(s)
    designEntityToStatementsThatModifyVarsMap[PKBDesignEntity::While].insert(
        res);
    mAllModifyStmts.insert(res);
  }
...
PKBStatement::SharedPtr
PKB::extractCallStatement(shared_ptr<Statement> &statement,
                          PKBGroup::SharedPtr &parentGroup) {
  // 1. create a PKBStatement
  PKBStatement::SharedPtr res = createPKBStatement(statement, parentGroup);
  shared_ptr<CallStatement> callStatement =
      static_pointer_cast<CallStatement>(statement);
  ...
  if (!procedureNameToProcedureMap.count(procedureName)) {
    ...
    vector<shared_ptr<Procedure>> simpleProcedures =
        programToExtract->getProcedures();
  ...
  addModifiedVariable(PKBDesignEntity::Call,
                      procedureCalled->getModifiedVariables());

  if (procedureCalled->getModifiedVariables().size() > 0) {
    // the procedure call modifies variable(s) within
    designEntityToStatementsThatModifyVarsMap[PKBDesignEntity::Call].insert(
        res);
    mAllModifyStmts.insert(res);
  }
...
void PKB::addStatement(PKBStatement::SharedPtr &statement,
                       PKBDesignEntity designEntity) {
  mStatements[designEntity].emplace_back(statement);

  // also put it in the global bucket list
  if (designEntity != PKBDesignEntity::Procedure &&
      designEntity != PKBDesignEntity::AllExceptProcedure) {
    mStatements[PKBDesignEntity::AllExceptProcedure].emplace_back(statement);
  }
}

inline void PKB::addUsedVariable(PKBDesignEntity designEntity,
                                 PKBVariable::SharedPtr &variable) {
...
void PKB::addUsedVariable(PKBDesignEntity designEntity,
                          set<PKBVariable::SharedPtr> &variables) {
  for (auto v : variables) {
    addUsedVariable(designEntity, v);
  }
}

void PKB::addModifiedVariable(PKBDesignEntity designEntity,
                              PKBVariable::SharedPtr &variable) {
...
PKBStatement::SharedPtr
PKB::createPKBStatement(shared_ptr<Statement> &statement,
                        PKBGroup::SharedPtr &parentGroup) {
...
PKBGroup::SharedPtr PKB::createPKBGroup(PKBStatement::SharedPtr &ownerStatement,
                                        PKBGroup::SharedPtr &parentGroup) {
  // create group
...
PKBGroup::SharedPtr
PKB::createPKBGroup(string &name, PKBStatement::SharedPtr &ownerStatement) {
  // create group
...
const unordered_map<string, PKBVariable::SharedPtr> &
PKB::getAllVariablesMap() const {
  return mVariables;
}
...
vector<string> PKB::getIdentifiers(shared_ptr<ConditionalExpression> expr) {
  ...
  while (!queue.empty()) {
    ...
    case ConditionalType::BOOLEAN: {
      shared_ptr<BooleanExpression> bln =
          static_pointer_cast<BooleanExpression>(e);
      // recurse down
    ...
    case ConditionalType::RELATIONAL: {
      shared_ptr<RelationalExpression> reln =
          static_pointer_cast<RelationalExpression>(e);
      ...
      // inefficient way since we convert from vector to set multiple times, but
      // for now it will do

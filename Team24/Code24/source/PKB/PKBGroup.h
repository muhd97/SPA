...
public:
  ...
  vector<PKBGroup::SharedPtr>
      mChildGroups; // vector of shared pointer to all of it's child groups.

  // returns index (statement number) of statement which owns this group
  int getOwner() { return mOwnerIndex; }
  ...
  vector<int> getMembers(PKBDesignEntity s) { return mMembers[s]; }

  vector<PKBGroup::SharedPtr> getChildGroups() { return mChildGroups; }

  set<PKBVariable::SharedPtr> getUsedVariables() { return mUses; }

  set<PKBVariable::SharedPtr> getModifiedVariables() { return mModifies; }
  ...
  void addMember(int statementIndex, PKBDesignEntity designEntity) {
    ...
    if (designEntity != PKBDesignEntity::AllExceptProcedure) {
      mMembers[PKBDesignEntity::AllExceptProcedure].emplace_back(
          statementIndex);
    }
  }

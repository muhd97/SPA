...
public:
  ...
  // of 'while' stmts
  set<pair<int, int>> getParents(PKBDesignEntity parentType,
  ...
  set<pair<int, int>> getParentsT(PKBDesignEntity parentType,
                                  PKBDesignEntity childType);
  ...
  unordered_set<int>
  getAllChildAndSubChildrenOfGivenType(PKBStatement::SharedPtr targetParent,
                                       PKBDesignEntity targetChildrenType);

  /* Use for Parent*(INT, synonym) */
  unordered_set<int> getParentTIntSyn(int statementNo,
                                      PKBDesignEntity targetChildrenType);
  ...
  unordered_set<int> getParentTSynInt(PKBDesignEntity targetParentType,
                                      int childStatementNo);

  /* Use for Parent*(synonym1, synonym2) */
  set<pair<int, int>> getParentTSynSyn(PKBDesignEntity parentType,
                                       PKBDesignEntity childType);
  ...
  set<pair<int, int>> getBeforePairs(PKBDesignEntity beforeType,
  ...
  unordered_set<int> getFollowsTIntegerSyn(PKBDesignEntity childType,
                                           int parentStmtNo);
  ...
  unordered_set<int> getFollowsTSynInteger(PKBDesignEntity parentType,
                                           int childStmtNo);

  /* Use for Follows*(s1, s2) */
  set<pair<int, int>> getFollowsTSynSyn(PKBDesignEntity leftType,
  ...
  vector<int>
  getModifiers(PKBDesignEntity entityType); /* Get all stmts of a given type
  ...
  getStatementsByPKBDesignEntity(PKBDesignEntity pkbDe) const;
  ...
  vector<pair<int, string>> matchPartialPattern(string &LHS,
                                                shared_ptr<Expression> &RHS);
  // For pattern a("_", EXPR) or pattern a(IDENT, EXPR)
  vector<pair<int, string>> matchExactPattern(string &LHS,
                                              shared_ptr<Expression> &RHS);

protected:
  PQLEvaluator(PKB::SharedPtr pPKB) { mpPKB = pPKB; }
  ...
  bool isContainerType(PKBDesignEntity s) {
    return s == PKBDesignEntity::If || s == PKBDesignEntity::While ||
           s == PKBDesignEntity::Procedure ||
           s == PKBDesignEntity::AllExceptProcedure;
  }

  bool getStatementBefore(PKBStatement::SharedPtr &statementAfter,
                          PKBStatement::SharedPtr &result);
  bool getStatementAfter(PKBStatement::SharedPtr &statementBefore,
                         PKBStatement::SharedPtr &result);
  ...
  void addParentStmts(vector<PKBStatement::SharedPtr> &stmts) {
    ...
    vector<PKBStatement::SharedPtr> ifStmts =
        mpPKB->getStatements(PKBDesignEntity::If);
    vector<PKBStatement::SharedPtr> whileStmts =
        mpPKB->getStatements(PKBDesignEntity::While);
  ...
  bool hasEligibleChildRecursive(PKBGroup::SharedPtr grp,
  ...
  bool checkForSubTree(vector<string> &queryInOrder,
                       vector<string> &assignInOrder);
  bool checkForExactTree(vector<string> &queryInOrder,
                         vector<string> &assignInOrder);
};

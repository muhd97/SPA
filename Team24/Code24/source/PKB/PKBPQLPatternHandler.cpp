...
// instead
vector<pair<int, string>> PKBPQLPatternHandler::matchAnyPattern(string& LHS)
{
...
// if you want to use a(IDENT, EXPR) or a("_", EXPR), use matchExactPattern
// instead
vector<pair<int, string>> PKBPQLPatternHandler::matchPartialPattern(string& LHS, shared_ptr<Expression>& RHS)
{
	vector<PKBStmt::SharedPtr > assignStmts = mpPKB->getStatements(PKBDesignEntity::Assign);
	vector<pair<int, string>> res;
...
// instead
vector<pair<int, string>> PKBPQLPatternHandler::matchExactPattern(string& LHS, shared_ptr<Expression>& RHS)
{
	vector<PKBStmt::SharedPtr > assignStmts = mpPKB->getStatements(PKBDesignEntity::Assign);
	vector<pair<int, string>> res;
...
vector<string> PKBPQLPatternHandler::inOrderTraversalHelper(shared_ptr<Expression> expr)
{
...
vector<string> PKBPQLPatternHandler::preOrderTraversalHelper(shared_ptr<Expression> expr)
{
	vector<string> res;	// using a set to prevent duplicates
	vector<shared_ptr < Expression>> queue = { expr
	};
...
bool PKBPQLPatternHandler::checkForSubTree(vector<string>& queryInOrder, vector<string>& assignInOrder)
{
...
bool PKBPQLPatternHandler::checkForExactTree(vector<string>& queryInOrder, vector<string>& assignInOrder)
{

...
class PKBPQLNextBipHandler {
public:
    using SharedPtr = std::shared_ptr<PKBPQLNextBipHandler>;

    static SharedPtr create(PKB::SharedPtr pkb)
    {
        return SharedPtr(new PKBPQLNextBipHandler(pkb));
    }
...
private:
    ...
    PKBPQLNextBipHandler(PKB::SharedPtr pkb) {
        mpPKB = pkb;
    };
};
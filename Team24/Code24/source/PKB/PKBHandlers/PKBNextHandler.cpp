...
// NextT(p, q)
void PKBPQLNextHandler::getNextTStatementList(vector<shared_ptr<Statement>> list, StatementType from, StatementType to,
...
set<pair<int, int>> PKBPQLNextHandler::getNextT(shared_ptr<Program> program, StatementType from, StatementType to,
                                                int fromIndex, int toIndex, bool canExitEarly)
{
    ...
    for_each(execution::par_unseq, procs.begin(), procs.end(),
             [&from, &to, &fromIndex, &toIndex, &procs, &procSets, baseAddr](auto &&item) {
                 ...
                 set<int> seenP = {};
                 getNextTStatementList(procedure->getStatementList()->getStatements(), from, to, fromIndex, toIndex,
                                       &procSets[idx], &seenP, false);
             }

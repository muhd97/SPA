...
// ======================================================================================================
// Affects
bool PKBAffectsHandler::handleAffectsAssign(int index, bool includeAffectsT, map<string, set<int>> &lastModifiedTable,
...
void PKBAffectsHandler::handleAffectsReadCall(int index, map<string, set<int>> &lastModifiedTable)
{
    PKBStmt::SharedPtr stmt;
    if (mpPKB->getStatement(index, stmt))
    {
...
bool PKBAffectsHandler::getAffects(int leftInt, int rightInt, bool includeAffectsT)
{
    affectsList.clear();
    affectsTList.clear();
    ...
    if (leftInt == 0 && rightInt == 0)
    {
        ...
        {
            if (computeAffects(p.second, includeAffectsT, map<string, set<int>>(), shared_ptr<BasicBlock>(), true,
                               leftInt, rightInt))
            {
                return true;
            }
    ...
    return computeAffects(firstBlock, includeAffectsT, map<string, set<int>>(), shared_ptr<BasicBlock>(), true, leftInt,
                          rightInt);
}

// 5 cases: (int, syn) (syn, int) (syn, syn) (syn, _) (_, syn)
pair<set<pair<int, int>>, set<pair<int, int>>> PKBAffectsHandler::getAffects(bool includeAffectsT,
...
bool PKBAffectsHandler::computeAffects(const shared_ptr<BasicBlock> &basicBlock, bool includeAffectsT,
                                       ...
                                       bool terminateEarly, int leftInt, int rightInt)
{
    ...
    if (statements.size() == 0)
    {
        ...
        {
            return computeAffects(basicBlock->getNext().back(), includeAffectsT, lastModifiedTable, lastBlock,
    ...
    for (shared_ptr<CFGStatement> &stmt : statements)
    {
        ...
        else if (stmt->type == PKBDesignEntity::If)
        {
            ...
            if (computeAffects(ifBlock, includeAffectsT, lastModifiedTable, lastBlock, terminateEarly, leftInt,
                               rightInt))
            {
                return true;
            }
            if (computeAffects(elseBlock, includeAffectsT, lastModifiedTableCopy, lastBlock, terminateEarly, leftInt,
                               rightInt))
            {
                return true;
            }
            ...
            {
                return computeAffects(lastBlock->getNext().back(), includeAffectsT, lastModifiedTable, lastBlock,
                                      terminateEarly, leftInt, rightInt);
            }
        }
        else if (stmt->type == PKBDesignEntity::While)
        {
            ...
            if (computeAffects(nestedBlock, includeAffectsT, lastModifiedTableCopy2, lastBlock, terminateEarly, leftInt,
                               rightInt))
            {
                return true;
            };
            ...
            if (lastModifiedTable != lastModifiedTableCopy2 || affectsTcount != affectsTList.size())
            {
                do
                {
                    ...
                    if (computeAffects(nestedBlock, includeAffectsT, lastModifiedTableCopy2, lastBlock, terminateEarly,
                                       leftInt, rightInt))
                    {
                        return true;
                    };
            ...
            if (basicBlock->goNext)
            {
                return computeAffects(nextBlocks[1], includeAffectsT, lastModifiedTable, lastBlock, terminateEarly,
                                      leftInt, rightInt);
            }
            lastBlock = basicBlock;
            return false;
    ...
    if (basicBlock->goNext)
    {
        return computeAffects(basicBlock->getNext()[0], includeAffectsT, lastModifiedTable, lastBlock, terminateEarly,
                              leftInt, rightInt);
    }
    lastBlock = basicBlock;
    return false;

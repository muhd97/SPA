 ...
 * targetType (string) */
inline bool targetSynonymMatchesType(shared_ptr<SelectCl> selectCl,
                                     string targetType) {
  return selectCl->getDesignEntityTypeBySynonym(
             selectCl->targetSynonym->getValue()) == targetType;
}
...
inline bool targetSynonymNotDeclared(shared_ptr<SelectCl> selectCl) {
  return selectCl->synonymToParentDeclarationMap.find(
             selectCl->targetSynonym->getValue()) ==
...
inline bool targetSynonymMatchesMultipleTypes(shared_ptr<SelectCl> selectCl,
                                              initializer_list<string> list) {
  bool flag = false;
  string toMatch = selectCl->getDesignEntityTypeBySynonym(
      selectCl->targetSynonym->getValue());
...
/* Method to check if the target synonym in the select statement is found in its
 * suchThat OR pattern clauses */
...
inline bool
singleRefSynonymMatchesTargetSynonym(shared_ptr<Ref> &refToCheck,
                                     shared_ptr<SelectCl> &selectCl) {
  return refToCheck->getStringVal() == selectCl->targetSynonym->getValue();
}

inline bool givenSynonymMatchesMultipleTypes(shared_ptr<SelectCl> selectCl,
                                             string toCheck,
                                             initializer_list<string> list) {
  bool flag = false;
 ...
 * supported here!! */
inline PKBDesignEntity
resolvePQLDesignEntityToPKBDesignEntity(shared_ptr<DesignEntity> de) {
...
PQLProcessor::handleNoSuchThatOrPatternCase(shared_ptr<SelectCl> selectCl) {
  shared_ptr<Synonym> targetSynonym = selectCl->targetSynonym;
  shared_ptr<DesignEntity> de =
      selectCl->getParentDeclarationForSynonym(targetSynonym)
          ->getDesignEntity();
  ...
  if (de->getEntityTypeName() == PQL_VARIABLE) {
    ...
    for (auto &ptr : vars)
      toReturn.emplace_back(
          make_shared<VariableNameSingleResult>(ptr->getName()));
    return move(toReturn);
  }
...
void PQLProcessor::handleSuchThatClause(
    shared_ptr<SelectCl> selectCl, shared_ptr<SuchThatCl> suchThatCl,
    vector<shared_ptr<ResultTuple>> &toReturn) { // TODO IMPLEMENT
                              ...
                              if/while/assign/stmt/read/print. */
  {
    ...
    if (leftType == StmtRefType::UNDERSCORE) {
      throw "TODO: Handle Uses error case. Uses (_, x) cannot have first "
    ...
    if (leftType == StmtRefType::SYNONYM) {
      if (selectCl->getDesignEntityTypeBySynonym(
              usesCl->stmtRef->getStringVal()) == DesignEntity::CONSTANT) {
        throw "TODO: Handle error case. Uses(syn, ?), but syn is a Constant "
  ...
  {
    shared_ptr<ModifiesS> modifiesCl =
        static_pointer_cast<ModifiesS>(suchThatCl->relRef);
    ...
    if (stmtRef->getStmtRefType() == StmtRefType::INTEGER) {
      vector<string> variablesModifiedByStmtNo =
          evaluator->getModified(stmtRef->getIntVal());
      if (entRef->getEntRefType() == EntRefType::SYNONYM) {
        if (selectCl->getDesignEntityTypeBySynonym(entRef->getStringVal()) !=
            PQL_VARIABLE) { // Modifies (1, x), x is NOT a variable
      ...
      if (rightType == EntRefType::IDENT) {
        if (evaluator->checkModified(stmtRef->getIntVal(),
          ...
          tupleToAdd->insertKeyValuePair(ResultTuple::INTEGER_PLACEHOLDER,
                                         to_string(stmtRef->getIntVal()));
    ...
    if (stmtRef->getStmtRefType() == StmtRefType::SYNONYM) {
      // This is handling for both statement and procedure in Iteration 1. Need
      // to change to make sure procedures are handled in ModifiesP
      string leftSynonymKey = stmtRef->getStringVal();

      /* Modifies (syn, v) */
      if (rightType == EntRefType::SYNONYM) {
        if (selectCl->getDesignEntityTypeBySynonym(entRef->getStringVal()) !=
            PQL_VARIABLE) { // Modifies (s, x), x is NOT a variable
        ...
        if (selectCl->getDesignEntityTypeBySynonym(leftSynonymKey) !=
      ...
      if (rightType == EntRefType::IDENT) {
        ...
        if (selectCl->getDesignEntityTypeBySynonym(leftSynonymKey) ==
          ...
          for (auto &p :
               evaluator->mpPKB
                   ->mVariableNameToProceduresThatModifyVarsMap[identVarName]) {
            shared_ptr<ResultTuple> tupleToAdd = make_shared<ResultTuple>();
  ...
  case RelRefType::MODIFIES_P: /* Modifies("IDENT", ...) where IDENT must be a
                                  procedure */
  {
    shared_ptr<ModifiesP> modifiesCl =
        static_pointer_cast<ModifiesP>(suchThatCl->relRef);
    ...
    if (rightType == EntRefType::SYNONYM) {
      if (selectCl->getDesignEntityTypeBySynonym(entRefRight->getStringVal()) !=
          PQL_VARIABLE) { // Modifies (s, x), x is NOT a variable
    ...
    if (entRefRight->getEntRefType() == EntRefType::IDENT) {
      if (evaluator->checkModifiedByProcName(leftArg,
                                             entRefRight->getStringVal())) {
        shared_ptr<ResultTuple> tupleToAdd = make_shared<ResultTuple>();
        string rightArg = entRefRight->getStringVal();
        tupleToAdd->insertKeyValuePair(ResultTuple::IDENT_PLACEHOLDER, leftArg);
        tupleToAdd->insertKeyValuePair(ResultTuple::IDENT_PLACEHOLDER,
                                       rightArg);
  ...
  case RelRefType::PARENT: {
    shared_ptr<Parent> parentCl =
        static_pointer_cast<Parent>(suchThatCl->relRef);
  ...
  case RelRefType::PARENT_T: {
    shared_ptr<ParentT> parentCl =
        static_pointer_cast<ParentT>(suchThatCl->relRef);
    StmtRefType leftType = parentCl->stmtRef1->getStmtRefType();
    StmtRefType rightType = parentCl->stmtRef2->getStmtRefType();
  ...
  case RelRefType::FOLLOWS: {
    ...
    shared_ptr<Follows> followsCl =
        static_pointer_cast<Follows>(suchThatCl->relRef);
    ...
    if (leftType == StmtRefType::INTEGER) {
      ...
      vector<int> statementsFollowedByStmtNo = evaluator->getAfter(
          PKBDesignEntity::AllExceptProcedure, stmtRef1->getIntVal());
      shared_ptr<Synonym> targetSynonym = selectCl->targetSynonym;
      if (rightType == StmtRefType::SYNONYM) {
        shared_ptr<Declaration> &parentDecl =
            selectCl->synonymToParentDeclarationMap[rightSynonymKey];
        PKBDesignEntity pkbDe = resolvePQLDesignEntityToPKBDesignEntity(
            parentDecl->getDesignEntity());
        /* Follows (1, syn), syn is NOT a variable */
        // if (selectCl->getDesignEntityTypeBySynonym(stmtRef2->getStringVal())
        // != VARIABLE) {
        //    throw "TODO: Handle error case. Follows(1, p), but p is not a
      ...
      // maybe later need to optimize this to return just 1 result if there are
      // any at all
      if (rightType == StmtRefType::UNDERSCORE) {
        // if (evaluator->checkFollowed(stmtRef1->getIntVal())) {
        /* Create the result tuple */
        for (auto &s : evaluator->getAfter(PKBDesignEntity::AllExceptProcedure,
                                           stmtRef1->getIntVal())) {
          shared_ptr<ResultTuple> tupleToAdd = make_shared<ResultTuple>();
          tupleToAdd->insertKeyValuePair(ResultTuple::INTEGER_PLACEHOLDER,
                                         to_string(s));
      ...
      if (rightType == StmtRefType::INTEGER) {
        ...
        for (auto &s :
             evaluator->getAfter(PKBDesignEntity::AllExceptProcedure, s1)) {
          if (s == s2) {
            shared_ptr<ResultTuple> tupleToAdd = make_shared<ResultTuple>();
            tupleToAdd->insertKeyValuePair(ResultTuple::INTEGER_PLACEHOLDER,
                                           to_string(s1));
    ...
    if (leftType == StmtRefType::SYNONYM) {
      ...
      if (!givenSynonymMatchesMultipleTypes(
              selectCl, leftSynonymKey,
              {DesignEntity::ASSIGN, DesignEntity::CALL, DesignEntity::IF,
               DesignEntity::PRINT, DesignEntity::READ, DesignEntity::STMT,
               DesignEntity::WHILE})) {
        throw "Follows Error: The synonyms in a Follows relationship must be "
              "statements!";
      }

      /* Follows(s, 5) */
      if (rightType == StmtRefType::INTEGER) {
        shared_ptr<Declaration> &parentDecl1 =
            selectCl->synonymToParentDeclarationMap[leftSynonymKey];
        PKBDesignEntity pkbDe1 = resolvePQLDesignEntityToPKBDesignEntity(
            parentDecl1->getDesignEntity());
      ...
      if (rightType == StmtRefType::SYNONYM) {
        ...
        // Special case: both synonyms in the Follows are the same: return empty
        // result by not adding any tuples
        ...
        shared_ptr<Declaration> &parentDecl2 =
            selectCl->synonymToParentDeclarationMap[rightSynonymKey];
        PKBDesignEntity pkbDe2 = resolvePQLDesignEntityToPKBDesignEntity(
            parentDecl2->getDesignEntity());
        ...
        for (auto &sPair : sPairs) {
          ...
          /* Map the value returned to this particular synonym. */
          tupleToAdd->insertKeyValuePair(leftSynonymKey,
                                         to_string(sPair.first));
          tupleToAdd->insertKeyValuePair(rightSynonymKey,
                                         to_string(sPair.second));
      ...
      if (rightType == StmtRefType::UNDERSCORE) {
        ...
        PKBDesignEntity pkbDe1 = resolvePQLDesignEntityToPKBDesignEntity(
            parentDecl1->getDesignEntity());
        for (auto &s : evaluator->getBefore(
                 pkbDe1, PKBDesignEntity::AllExceptProcedure)) {
          shared_ptr<ResultTuple> tupleToAdd = make_shared<ResultTuple>();
    ...
    if (leftType == StmtRefType::UNDERSCORE) {
      ...
      if (rightType == StmtRefType::INTEGER) {
        for (auto &s : evaluator->getBefore(PKBDesignEntity::AllExceptProcedure,
                                            stmtRef2->getIntVal())) {
          shared_ptr<ResultTuple> tupleToAdd = make_shared<ResultTuple>();
          tupleToAdd->insertKeyValuePair(ResultTuple::INTEGER_PLACEHOLDER,
                                         to_string(s));
      ...
      if (rightType == StmtRefType::SYNONYM) {
        ...
        for (auto &s :
             evaluator->getAfter(PKBDesignEntity::AllExceptProcedure, pkbDe)) {
          shared_ptr<ResultTuple> tupleToAdd = make_shared<ResultTuple>();
  ...
  case RelRefType::FOLLOWS_T: {
    shared_ptr<FollowsT> followstCl =
        static_pointer_cast<FollowsT>(suchThatCl->relRef);
...
void PQLProcessor::handleUsesSFirstArgInteger(
    shared_ptr<SelectCl> &selectCl, shared_ptr<UsesS> &usesCl,
  ...
  vector<string> variablesUsedByStmtNo =
      evaluator->getUsed(stmtRef->getIntVal());
  shared_ptr<Synonym> targetSynonym = selectCl->targetSynonym;
...
void PQLProcessor::handleUsesSFirstArgSyn(
    shared_ptr<SelectCl> &selectCl, shared_ptr<UsesS> &usesCl,
    vector<shared_ptr<ResultTuple>> &toReturn) {
  StmtRefType leftType = usesCl->stmtRef->getStmtRefType();
  EntRefType rightType = usesCl->entRef->getEntRefType();
  ...
  if (rightType == EntRefType::SYNONYM) {
    ...
    if (selectCl->getDesignEntityTypeBySynonym(rightSynonymKey) !=
        DesignEntity::VARIABLE) {
      throw "TODO: Handle error case. Uses(syn, v), but v is not a variable "
            "delcaration.\n";
    }
    ...
    if (selectCl->getDesignEntityTypeBySynonym(leftSynonymKey) !=
      ...
      shared_ptr<Declaration> &parentDecl =
          selectCl->synonymToParentDeclarationMap[stmtRefLeft->getStringVal()];
      PKBDesignEntity pkbDe = resolvePQLDesignEntityToPKBDesignEntity(
          parentDecl->getDesignEntity());
  ...
  if (rightType == EntRefType::IDENT) {
    shared_ptr<Declaration> &parentDecl =
        selectCl->synonymToParentDeclarationMap[stmtRefLeft->getStringVal()];
    PKBDesignEntity pkbDe =
        resolvePQLDesignEntityToPKBDesignEntity(parentDecl->getDesignEntity());
    ...
    if (selectCl->getDesignEntityTypeBySynonym(leftSynonymKey) ==
      ...
      for (auto &p :
           evaluator->mpPKB
               ->variableNameToProceduresThatUseVarMap[identVarName]) {
        shared_ptr<ResultTuple> tupleToAdd = make_shared<ResultTuple>();
...
void PQLProcessor::handleUsesPFirstArgIdent(
    shared_ptr<SelectCl> &selectCl, shared_ptr<UsesP> &usesCl,
    vector<shared_ptr<ResultTuple>> &toReturn) {
  ...
  if (usesCl->entRef2->getEntRefType() == EntRefType::SYNONYM) {
    ...
    if (selectCl->getDesignEntityTypeBySynonym(
            usesCl->entRef2->getStringVal()) != DesignEntity::VARIABLE) {
    ...
    for (auto &s :
         evaluator->getUsedByProcName(usesCl->entRef1->getStringVal())) {
      shared_ptr<ResultTuple> tupleToAdd = make_shared<ResultTuple>();
...
void PQLProcessor::handleParentFirstArgInteger(
    shared_ptr<SelectCl> &selectCl, shared_ptr<Parent> &parentCl,
    vector<shared_ptr<ResultTuple>> &toReturn) {
   ...
   * Stmt/Read/Print/Call/While/If/Assign. Cannot be Procedure/Constant/Variable
   */
  if (rightArg->getStmtRefType() == StmtRefType::SYNONYM) {
    ...
    if (givenSynonymMatchesMultipleTypes(selectCl, rightSynonym,
      ...
      throw "TODO: Handle error case. Parent(INTEGER, syn), but syn is "
            "declared as Procedure, Constant or Variable. These DesignEntity "
            "types have no parents.\n";
      return;
    }

    PKBDesignEntity rightArgType = resolvePQLDesignEntityToPKBDesignEntity(
        selectCl->getDesignEntityTypeBySynonym(rightSynonym));
  ...
  if (rightArg->getStmtRefType() == StmtRefType::UNDERSCORE) {
    ...
    if (evaluator->mpPKB->getStatement(leftArgInteger, stmt)) {
      if (evaluator
              ->getChildren(PKBDesignEntity::AllExceptProcedure,
                            stmt->getIndex())
              .size() > 0u) {
        shared_ptr<ResultTuple> tupleToAdd = make_shared<ResultTuple>();
        /* Map the value returned to this particular synonym. */
        tupleToAdd->insertKeyValuePair(ResultTuple::INTEGER_PLACEHOLDER,
                                       to_string(leftArgInteger));
        /* Add this tuple into the vector to tuples to return. */
        toReturn.emplace_back(move(tupleToAdd));
      }
    }
  ...
  if (rightArg->getStmtRefType() == StmtRefType::INTEGER) {
    ...
    if (evaluator->mpPKB->getStatement(leftArgInteger, stmt)) {
      set<int> &childrenIds = evaluator->getChildren(
          PKBDesignEntity::AllExceptProcedure, stmt->getIndex());

      if (childrenIds.size() > 0u &&
          (childrenIds.find(rightArgInteger) != childrenIds.end())) {
        shared_ptr<ResultTuple> tupleToAdd = make_shared<ResultTuple>();
        /* Map the value returned to this particular synonym. */
        tupleToAdd->insertKeyValuePair(ResultTuple::INTEGER_PLACEHOLDER,
                                       to_string(leftArgInteger));
        tupleToAdd->insertKeyValuePair(ResultTuple::INTEGER_PLACEHOLDER,
                                       to_string(rightArgInteger));
        /* Add this tuple into the vector to tuples to return. */
        toReturn.emplace_back(move(tupleToAdd));
      }
    }
...
void PQLProcessor::handleParentFirstArgSyn(
    shared_ptr<SelectCl> &selectCl, shared_ptr<Parent> &parentCl,
    vector<shared_ptr<ResultTuple>> &toReturn) {
  shared_ptr<StmtRef> &leftArg = parentCl->stmtRef1;
  shared_ptr<StmtRef> &rightArg = parentCl->stmtRef2;
  ...
  PKBDesignEntity leftArgType = resolvePQLDesignEntityToPKBDesignEntity(
      selectCl->getDesignEntityTypeBySynonym(leftSynonym));
  ...
  if (rightArg->getStmtRefType() == StmtRefType::SYNONYM) {
    ...
    if (givenSynonymMatchesMultipleTypes(selectCl, rightSynonym,
                                         {DesignEntity::PROCEDURE,
                                          DesignEntity::CONSTANT,
                                          DesignEntity::VARIABLE})) {
      throw "TODO: Handle error case. Parent(syn, s), but s is declared as "
            "Procedure, Constant or Variable. These DesignEntity types have no "
            "parents.\n";
      return;
    }
...
void PQLProcessor::handleParentFirstArgUnderscore(
    shared_ptr<SelectCl> &selectCl, shared_ptr<Parent> &parentCl,
    vector<shared_ptr<ResultTuple>> &toReturn) {
  shared_ptr<StmtRef> &leftArg = parentCl->stmtRef1;
  shared_ptr<StmtRef> &rightArg = parentCl->stmtRef2;
  ...
  if (rightArg->getStmtRefType() == StmtRefType::INTEGER) {
    ...
    if (!evaluator
             ->getParents(PKBDesignEntity::AllExceptProcedure, rightArgInteger)
             .empty()) {
      /* Create the result tuple */
      shared_ptr<ResultTuple> tupleToAdd = make_shared<ResultTuple>();
      /* Map the value returned to this particular synonym. */
...
void PQLProcessor::handleParentTFirstArgInteger(
    shared_ptr<SelectCl> &selectCl, shared_ptr<ParentT> &parentCl,
    vector<shared_ptr<ResultTuple>> &toReturn) {
  shared_ptr<StmtRef> &leftArg = parentCl->stmtRef1;
  shared_ptr<StmtRef> &rightArg = parentCl->stmtRef2;
...
void PQLProcessor::handleParentTFirstArgSyn(
    shared_ptr<SelectCl> &selectCl, shared_ptr<ParentT> &parentCl,
    vector<shared_ptr<ResultTuple>> &toReturn) {
  shared_ptr<StmtRef> &leftArg = parentCl->stmtRef1;
  shared_ptr<StmtRef> &rightArg = parentCl->stmtRef2;
  ...
  if (rightArg->getStmtRefType() == StmtRefType::SYNONYM) {
    ...
    if (givenSynonymMatchesMultipleTypes(selectCl, rightSynonym,
                                         {DesignEntity::PROCEDURE,
                                          DesignEntity::CONSTANT,
                                          DesignEntity::VARIABLE})) {
      throw "TODO: Handle error case. ParentT(syn, s), but s is declared as "
            "Procedure, Constant or Variable. These DesignEntity types don't "
            "have parents.\n";
      return;
    }
  ...
  if (rightArg->getStmtRefType() == StmtRefType::INTEGER) {
    ...
    for (const int &x :
         evaluator->getParentTSynInt(leftArgType, rightArgInteger)) {
      shared_ptr<ResultTuple> tupleToAdd = make_shared<ResultTuple>();
      /* Map the value returned to this particular synonym. */
      tupleToAdd->insertKeyValuePair(leftSynonym, to_string(x));
      /* Add this tuple into the vector to tuples to return. */
...
void PQLProcessor::handleParentTFirstArgUnderscore(
    shared_ptr<SelectCl> &selectCl, shared_ptr<ParentT> &parentCl,
    vector<shared_ptr<ResultTuple>> &toReturn) {
  shared_ptr<StmtRef> &leftArg = parentCl->stmtRef1;
  shared_ptr<StmtRef> &rightArg = parentCl->stmtRef2;
...
void PQLProcessor::handleFollowsTFirstArgSyn(
    shared_ptr<SelectCl> &selectCl, shared_ptr<FollowsT> &followsTCl,
    vector<shared_ptr<ResultTuple>> &toReturn) {
  ...
  if (rightArg->getStmtRefType() == StmtRefType::SYNONYM) {
    ...
    shared_ptr<Declaration> &parentDecl1 =
        selectCl->synonymToParentDeclarationMap[leftSynonymKey];
    PKBDesignEntity pkbDe1 =
        resolvePQLDesignEntityToPKBDesignEntity(parentDecl1->getDesignEntity());

    shared_ptr<Declaration> &parentDecl2 =
        selectCl->synonymToParentDeclarationMap[rightSynonymKey];
    PKBDesignEntity pkbDe2 =
        resolvePQLDesignEntityToPKBDesignEntity(parentDecl2->getDesignEntity());
...
void PQLProcessor::handleFollowsTFirstArgInteger(
    shared_ptr<SelectCl> &selectCl, shared_ptr<FollowsT> &followsTCl,
    vector<shared_ptr<ResultTuple>> &toReturn) {
  shared_ptr<StmtRef> &stmtRef1 = followsTCl->stmtRef1;
  shared_ptr<StmtRef> &stmtRef2 = followsTCl->stmtRef2;
  ...
  if (rightType == StmtRefType::SYNONYM) {
    ...
    if (givenSynonymMatchesMultipleTypes(selectCl, rightSynonymKey,
                                         {DesignEntity::PROCEDURE,
                                          DesignEntity::CONSTANT,
                                          DesignEntity::VARIABLE})) {
      throw "Follows*(INT, s1) but s1 is not declared a type of statement. "
            "Follows*() is only defined for statements\n";
      return;
    }
    ...
    shared_ptr<Declaration> &decl =
        selectCl->synonymToParentDeclarationMap[rightSynonymKey];
    PKBDesignEntity pkbde =
        resolvePQLDesignEntityToPKBDesignEntity(decl->getDesignEntity());
    for (auto &s :
         evaluator->getFollowsTIntegerSyn(pkbde, stmtRef1->getIntVal())) {
  ...
  // check if follows(int, int) is true
    ...
    if (evaluator->getFollowsTIntegerInteger(s1, s2)) {
      shared_ptr<ResultTuple> tupleToAdd = make_shared<ResultTuple>();
      tupleToAdd->insertKeyValuePair(ResultTuple::INTEGER_PLACEHOLDER,
                                     ResultTuple::INTEGER_PLACEHOLDER);
      toReturn.emplace_back(tupleToAdd);
    }
  }
}

void PQLProcessor::handleFollowsTFirstArgUnderscore(
    shared_ptr<SelectCl> &selectCl, shared_ptr<FollowsT> &followsTCl,
    vector<shared_ptr<ResultTuple>> &toReturn) {
  shared_ptr<StmtRef> &stmtRef1 = followsTCl->stmtRef1;
  shared_ptr<StmtRef> &stmtRef2 = followsTCl->stmtRef2;
...
void PQLProcessor::handlePatternClause(
    shared_ptr<SelectCl> selectCl, shared_ptr<PatternCl> patternCl,
    vector<shared_ptr<ResultTuple>> &toReturn) {
  // LHS
  ...
  if (exprSpec->isAnything) {
    ...
    pairsStmtIndexAndVariables =
        evaluator->matchPartialPattern(LHS, exprSpec->expression);
  } else {
    pairsStmtIndexAndVariables =
        evaluator->matchExactPattern(LHS, exprSpec->expression);
  }
  for (auto &pair : pairsStmtIndexAndVariables) {
    shared_ptr<ResultTuple> tupleToAdd = make_shared<ResultTuple>();
    /* Map the value returned to this particular synonym. */
    tupleToAdd->insertKeyValuePair(patternCl->synonym->getValue(),
...
void PQLProcessor::joinResultTuples(
    vector<shared_ptr<ResultTuple>> leftResults,
  ...
  for (auto &leftPtr : leftResults) {
    for (auto &rightPtr : rightResults) {
      ...
      for (const string &key : joinKeys) {
        if (!((leftPtr->synonymKeyAlreadyExists(key) &&
               rightPtr->synonymKeyAlreadyExists(key)) &&
      ...
      if (allJoinKeysMatch) {
        shared_ptr<ResultTuple> toAdd =
            make_shared<ResultTuple>(leftPtr->synonymKeyToValMap.size() +
                                     rightPtr->synonymKeyToValMap.size());
...
void PQLProcessor::cartesianProductResultTuples(
    vector<shared_ptr<ResultTuple>> leftResults,
    vector<shared_ptr<ResultTuple>> rightResults,
    vector<shared_ptr<ResultTuple>> &newResults) {
...
void setCommonSynonymToJoinOn(shared_ptr<SuchThatCl> suchThatCl,
...
getSetOfSynonymsToJoinOn(shared_ptr<SuchThatCl> suchThatCl,
                         shared_ptr<PatternCl> patternCl) {
...
unordered_set<string>
getSetOfSynonymsToJoinOn(shared_ptr<SuchThatCl> suchThatCl1,
                         shared_ptr<SuchThatCl> suchThatCl2) {
  unordered_set<string> toReturn;
  toReturn.reserve(4); /* At most 4 keys to join on */
...
inline bool
allSynonymsInSuchThatClausesAreDeclared(shared_ptr<SelectCl> &selectCl) {
...
inline bool
allSynonymsInPatternClausesAreDeclared(shared_ptr<SelectCl> &selectCl) {
...
vector<shared_ptr<Result>>
PQLProcessor::processPQLQuery(shared_ptr<SelectCl> selectCl) {

  /* Pre-Validate PQLQuery first to catch simple errors like a synonym not being
   * declared first. */
  ...
  if (selectCl->hasSuchThatClauses()) {
    ...
    for (unsigned int i = 0; i < selectCl->suchThatClauses.size(); i++) {
      if (i == 0) {
        try {
          handleSuchThatClause(selectCl, selectCl->suchThatClauses[i],
                               suchThatReturnTuples);
        ...
        unordered_set<string> &setOfSynonymsToJoinOn = getSetOfSynonymsToJoinOn(
            previousSelectCl, selectCl->suchThatClauses[i]);

        if (!setOfSynonymsToJoinOn.empty())
          joinResultTuples(suchThatReturnTuples, currSuchThatRes,
                           setOfSynonymsToJoinOn, joinedRes);
        else
          cartesianProductResultTuples(suchThatReturnTuples, currSuchThatRes,
                                       joinedRes);
  ...
  if (selectCl->hasPatternClauses() && selectCl->hasSuchThatClauses()) {
    ...
    unordered_set<string> &setOfSynonymsToJoinOn = getSetOfSynonymsToJoinOn(
        selectCl->suchThatClauses[0], selectCl->patternClauses[0]);

    if (setOfSynonymsToJoinOn
            .empty()) { // no need to join, take cartesian product
      cout << "NO NEED TO JOIN\n";
      cartesianProductResultTuples(suchThatReturnTuples, patternReturnTuples,
                                   combinedTuples);
    } else {
      joinResultTuples(suchThatReturnTuples, patternReturnTuples,
                       setOfSynonymsToJoinOn, combinedTuples);
    }

    if (!targetSynonymIsInClauses(selectCl)) {
      return combinedTuples.size() <= 0
                 ? res
                 : handleNoSuchThatOrPatternCase(move(selectCl));
    }

    /* STEP 4a: After joining or taking cartesian product, find values for the
     * target synonym and return. */
    ...
    for (auto &tuple : combinedTuples) {
      if (!stringIsInsideSet(combinedResults, tuple->get(targetSynonymVal))) {
        res.emplace_back(
            make_shared<StringSingleResult>(tuple->get(targetSynonymVal)));
  ...
  vector<shared_ptr<ResultTuple>> &finalTuples = !selectCl->hasPatternClauses()
                                                     ? suchThatReturnTuples
                                                     : patternReturnTuples;

  if (!targetSynonymIsInClauses(selectCl)) {
    return finalTuples.empty() ? move(res)
                               : handleNoSuchThatOrPatternCase(move(selectCl));
  }

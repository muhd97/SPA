...
int getNextStatementNumber() { return statementCounter++; }
...
public:
  ...
  void addProc(string procName) { procedures.insert(procName); }
  ...
  bool isProcedureCallsValid() {
    ...
    for (string proc : proceduresInvoked) {
      if (!this->isProcAlreadyDeclared(proc)) {
        cout << "Error: Call to undeclared procedure '" + proc +
                    "' is not allowed."
             << endl;
        return false;
      }
  ...
  map<string, vector<string>> getCallGraph() { return this->callGraph; }
...
public:
  ...
  void error(vector<SimpleTokenType> expectedTypes, SimpleToken actual) {
    ...
    cout << "] but got '" << getSimpleTokenLabel(actual) << "' instead."
         << endl;
  ...
  void errorKeyword(string expectedKeyword, SimpleToken actual) {
    ...
    cout << "Expected \"" << expectedKeyword << "\" but got "
         << getSimpleTokenLabel(actual) << "' instead." << endl;

    // TODO: (@jiachen) do proper error handling
    exit(0);
  }

  bool isEmpty() { return index == size; }
  ...
  SimpleToken eatKeyword(string expectedKeyword) {
    SimpleToken actualSimpleToken = peek();
    if (actualSimpleToken.type == SimpleTokenType::NAME &&
        actualSimpleToken.value == expectedKeyword) {
      advance();
    } else {
  ...
  shared_ptr<Statement> parseStatement(shared_ptr<Environment> env) {
    ...
    } else if (peek().type == SimpleTokenType::NAME &&
               peek().value == SIMPLE_READ) {
      return parseReadStatement();
    } else if (peek().type == SimpleTokenType::NAME &&
               peek().value == SIMPLE_PRINT) {
      return parsePrintStatement();
    } else if (peek().type == SimpleTokenType::NAME &&
               peek().value == SIMPLE_CALL) {
      return parseCallStatement(env);
    } else if (peek().type == SimpleTokenType::NAME &&
               peek().value == SIMPLE_WHILE) {
      return parseWhileStatement(env);
    } else if (peek().type == SimpleTokenType::NAME &&
               peek().value == SIMPLE_IF) {
  ...
  shared_ptr<ReadStatement> parseReadStatement() {
    ...
    return make_shared<ReadStatement>(index,
                                      make_shared<Identifier>(name.value));
  }
  ...
  shared_ptr<PrintStatement> parsePrintStatement() {
    ...
    SimpleToken name = eat(SimpleTokenType::NAME);
    eat(SimpleTokenType::SEMICOLON);
    return make_shared<PrintStatement>(index,
                                       make_shared<Identifier>(name.value));
  }
  ...
  shared_ptr<AssignStatement> parseAssignStatement() {
    ...
    eat(SimpleTokenType::SEMICOLON);
    return make_shared<AssignStatement>(
        index, make_shared<Identifier>(name.value), expr);
  }
  ...
  // || to see if it is a cond_expr, sound expr cannot contain these values This
  // should be faster than actually implementing full backtracking for one
  ...
  bool tryConditionalExpressionLookaheadHack() {
    ...
    if (SimpleTokens[index].type != SimpleTokenType::AND &&
        SimpleTokens[index].type != SimpleTokenType::OR) {
      return false;
    }
  ...
  shared_ptr<ConditionalExpression> parseConditionalExpression() {
    if (peek().type == SimpleTokenType::NOT) {
      ...
      return make_shared<NotExpression>(expr);
               ...
               tryConditionalExpressionLookaheadHack()) {
      eat(SimpleTokenType::LEFT_PAREN);
      ...
      return result;
               ...
               peek().type == SimpleTokenType::INTEGER ||
               peek().type == SimpleTokenType::LEFT_PAREN) {
      return parseRelationalExpression();
    } else {
      error({SimpleTokenType::NOT, SimpleTokenType::LEFT_PAREN,
             SimpleTokenType::NAME},
            peek());
      return NULL;
    }
  }
  ...
  shared_ptr<RelationalExpression> parseRelationalExpression() {
    ...
    if (peek().type == SimpleTokenType::GT) {
      ...
      error({SimpleTokenType::GT, SimpleTokenType::GTE, SimpleTokenType::LTE,
             SimpleTokenType::LT, SimpleTokenType::EQ, SimpleTokenType::NEQ},
            peek());
      return NULL;
    }
  ...
  shared_ptr<Expression> parseRelationalFactor() { return parseExpression(); }
  ...
  shared_ptr<Expression> parseExpressionPrime(shared_ptr<Expression> lhs) {
    ...
    return parseExpressionPrime(
        make_shared<CombinationExpression>(op, move(lhs), move(rhs)));
  }
  ...
  shared_ptr<Expression> parseFactor() {
    if (peek().type == SimpleTokenType::NAME) {
      ...
      error({SimpleTokenType::NAME, SimpleTokenType::INTEGER,

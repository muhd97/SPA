...
vector<shared_ptr<Result>> PQLProcessor::extractResultsNoClauses(shared_ptr<SelectCl> selectCl)
{
    ...
    if (numElements == 0)
    {
        ...
        if (selectCl->getTarget()->isBooleanReturnType())
            toReturn.emplace_back(make_shared<StringSingleResult>(Result::TRUE_STRING));
    ...
    for (const auto &x : elems)
    {
        ...
        {
            if (stringIsInsideSet(duplicateHelperSet, x->getSynonymString()))
                continue;
    ...
    for (auto tuple : res)
    {
        ...
        for (int i = 0; i < numElements; i++)
        {
            ...
            const string &val =
                (curr->getElementType() == ElementType::AttrRef)
                                       ...
                                       vector<shared_ptr<ResultTuple>> &toReturn)
{
                                ...
                                if/while/assign/stmt/read/print. */
    {
        ...
        if (usesSHandler->hasProcedureSynonym())
        {
            ...
            shared_ptr<UsesP> newUsesPCl = make_shared<UsesP>(
                make_shared<EntRef>(EntRefType::SYNONYM, usesSCl->stmtRef->getStringVal()), usesSCl->entRef);
            shared_ptr<UsesPHandler> usesPHandler =
                make_shared<UsesPHandler>(move(evaluator), move(selectCl), newUsesPCl);
    ...
    {
        shared_ptr<UsesPHandler> usesPHandler =
            make_shared<UsesPHandler>(move(evaluator), move(selectCl), static_pointer_cast<UsesP>(suchThatCl->relRef));
        usesPHandler->evaluate(move(toReturn));
        break;
    }
    case RelRefType::MODIFIES_S: /* Modifies(s, v) where s is a STATEMENT. */
    {
        ...
        shared_ptr<ModifiesSHandler> modifiesSHandler =
            make_shared<ModifiesSHandler>(move(evaluator), move(selectCl), modifiesSCl);

        if (modifiesSHandler->hasProcedureSynonym())
        {
            shared_ptr<ModifiesP> newModifiesPCl = make_shared<ModifiesP>(
                make_shared<EntRef>(EntRefType::SYNONYM, modifiesSCl->stmtRef->getStringVal()), modifiesSCl->entRef);
            shared_ptr<ModifiesPHandler> modifiesPHandler =
                make_shared<ModifiesPHandler>(move(evaluator), move(selectCl), newModifiesPCl);
    ...
    case RelRefType::MODIFIES_P: {
        shared_ptr<ModifiesPHandler> modifiesPHandler = make_shared<ModifiesPHandler>(
            move(evaluator), move(selectCl), static_pointer_cast<ModifiesP>(suchThatCl->relRef));
        modifiesPHandler->evaluate(move(toReturn));
        break;
    }
    case RelRefType::PARENT: {
        shared_ptr<ParentHandler> parentHandler = make_shared<ParentHandler>(
            move(evaluator), move(selectCl), static_pointer_cast<Parent>(suchThatCl->relRef));
        parentHandler->evaluate(move(toReturn));
        break;
    }
    case RelRefType::PARENT_T: {
        shared_ptr<ParentTHandler> parentTHandler = make_shared<ParentTHandler>(
            move(evaluator), move(selectCl), static_pointer_cast<ParentT>(suchThatCl->relRef));
        parentTHandler->evaluate(move(toReturn));
        break;
    }
    case RelRefType::FOLLOWS: {
        shared_ptr<FollowsHandler> followsHandler = make_shared<FollowsHandler>(
            move(evaluator), move(selectCl), static_pointer_cast<Follows>(suchThatCl->relRef));
        followsHandler->evaluate(move(toReturn));
        break;
    }

    case RelRefType::FOLLOWS_T: {
        shared_ptr<FollowsTHandler> followsTHandler = make_shared<FollowsTHandler>(
            move(evaluator), move(selectCl), static_pointer_cast<FollowsT>(suchThatCl->relRef));
        followsTHandler->evaluate(move(toReturn));
        break;
    }
    case RelRefType::CALLS: {
        shared_ptr<CallsHandler> callsHandler =
            make_shared<CallsHandler>(move(evaluator), move(selectCl), static_pointer_cast<Calls>(suchThatCl->relRef));
        callsHandler->evaluate(move(toReturn));
        break;
    }
    case RelRefType::CALLS_T: {
        shared_ptr<CallsTHandler> callsTHandler = make_shared<CallsTHandler>(
            move(evaluator), move(selectCl), static_pointer_cast<CallsT>(suchThatCl->relRef));
        callsTHandler->evaluate(move(toReturn));
        break;
    }
    case RelRefType::NEXT: {
        shared_ptr<NextHandler> nextHandler =
            make_shared<NextHandler>(move(evaluator), move(selectCl), static_pointer_cast<Next>(suchThatCl->relRef));
        nextHandler->evaluate(move(toReturn));
        break;
    }
    case RelRefType::NEXT_T: {
        shared_ptr<NextTHandler> nextTHandler =
            make_shared<NextTHandler>(move(evaluator), move(selectCl), static_pointer_cast<NextT>(suchThatCl->relRef));
        nextTHandler->evaluate(move(toReturn));
        break;
    }
    case RelRefType::NEXT_BIP: {
        shared_ptr<NextBipHandler> nextBipHandler = make_shared<NextBipHandler>(
            move(evaluator), move(selectCl), static_pointer_cast<NextBip>(suchThatCl->relRef));
        nextBipHandler->evaluate(move(toReturn));
        break;
    }
    case RelRefType::NEXT_BIP_T: {
        shared_ptr<NextBipTHandler> nextBipTHandler = make_shared<NextBipTHandler>(
            move(evaluator), move(selectCl), static_pointer_cast<NextBipT>(suchThatCl->relRef));
        nextBipTHandler->evaluate(move(toReturn));
        break;
    }
    case RelRefType::AFFECTS: {
        shared_ptr<AffectsHandler> affectsHandler = make_shared<AffectsHandler>(
            move(evaluator), move(selectCl), static_pointer_cast<Affects>(suchThatCl->relRef));
        affectsHandler->evaluate(move(toReturn));
        break;
    }
    case RelRefType::AFFECTS_T: {
        shared_ptr<AffectsTHandler> affectsTHandler = make_shared<AffectsTHandler>(
            move(evaluator), move(selectCl), static_pointer_cast<AffectsT>(suchThatCl->relRef));
        affectsTHandler->evaluate(move(toReturn));
        break;
    }
    case RelRefType::AFFECTS_BIP: {
        shared_ptr<AffectsBipHandler> affectsBipHandler = make_shared<AffectsBipHandler>(
            move(evaluator), move(selectCl), static_pointer_cast<AffectsBip>(suchThatCl->relRef));
        affectsBipHandler->evaluate(move(toReturn));
        break;
    }
    case RelRefType::AFFECTS_BIP_T: {
        shared_ptr<AffectsBipTHandler> affectsBipTHandler = make_shared<AffectsBipTHandler>(
            move(evaluator), move(selectCl), static_pointer_cast<AffectsBip>(suchThatCl->relRef));
...
void PQLProcessor::extractTargetSynonyms(vector<shared_ptr<Result>> &toReturn, const shared_ptr<ResultCl> &resultCl,
                                         vector<shared_ptr<ResultTuple>> &tuples, shared_ptr<SelectCl> &selectCl)
{
...
void PQLProcessor::extractTargetSynonymsBoolean(vector<shared_ptr<Result>> &toReturn,
                                                const shared_ptr<ResultCl> &resultCl,
                                                vector<shared_ptr<ResultTuple>> &tuples, shared_ptr<SelectCl> &selectCl)
{
    if (!tuples.empty())
        toReturn.emplace_back(make_shared<StringSingleResult>(Result::TRUE_STRING));
    else
        toReturn.emplace_back(make_shared<StringSingleResult>(Result::FALSE_STRING));
}

void PQLProcessor::extractTargetSynonymsSingle(vector<shared_ptr<Result>> &toReturn,
                                               const shared_ptr<ResultCl> &resultCl,
                                               vector<shared_ptr<ResultTuple>> &tuples, shared_ptr<SelectCl> &selectCl)
{
    if (tuples.empty())
    ...
    if (isAttrRef)
        attrRef = static_pointer_cast<AttrRef>(firstElem);
    if (!tuples[0]->synonymKeyAlreadyExists(targetSynonymVal))
        return;
    unordered_set<string> existingResults;
    for (auto &tuple : tuples)
    {
        const string &val =
            isAttrRef ? resolveAttrRef(targetSynonymVal, attrRef, selectCl, tuple) : tuple->get(targetSynonymVal);
...
void PQLProcessor::extractTargetSynonymsMultiple(vector<shared_ptr<Result>> &toReturn,
                                                 const shared_ptr<ResultCl> &resultCl,
                                                 vector<shared_ptr<ResultTuple>> &tuples,
                                                 shared_ptr<SelectCl> &selectCl)
{
    ...
    if (!dependentElementsAllExistInTupleKeys(tuples, independentElements, targetElems))
        return;

    /* If there are some elements that are not used in the suchThat/pattern/with clauses, we need to resolve them
     * separately. */
...
const string &PQLProcessor::resolveAttrRef(const string &syn, shared_ptr<AttrRef> &attrRef,
                                           const shared_ptr<SelectCl> &selectCl, shared_ptr<ResultTuple> &tup)
{
    return resolveAttrRef(tup->get(syn), attrRef, selectCl->getDesignEntityTypeBySynonym(syn));
}

const string &PQLProcessor::resolveAttrRef(const string &rawSynVal, shared_ptr<AttrRef> &attrRef,
                                           const shared_ptr<DesignEntity> &de)
{
...
void PQLProcessor::extractAllTuplesForSingleElement(const shared_ptr<SelectCl> &selectCl,
                                                    vector<shared_ptr<ResultTuple>> &toPopulate,
                                                    const shared_ptr<Element> &elem)
{
    ...
    if (de == PQL_PROCEDURE)
    {
        const set<shared_ptr<PKBProcedure>> &procedures = evaluator->getAllProcedures();
    ...
    vector<shared_ptr<PKBStmt>> stmts = pkbde == PKBDesignEntity::AllStatements
                                            ? evaluator->getAllStatements()
                                            : evaluator->getStatementsByPKBDesignEntity(pkbde);
...
void PQLProcessor::handleSingleEvalClause(shared_ptr<SelectCl> &selectCl, vector<shared_ptr<ResultTuple>> &toPopulate,
                                          const shared_ptr<EvalCl> evalCl)
{
...
void PQLProcessor::handleClauseGroup(shared_ptr<SelectCl> &selectCl, vector<shared_ptr<ResultTuple>> &toPopulate,
                                     const shared_ptr<ClauseGroup> &clauseGroup)
{
    ...
    for (const auto &clPtr : clauseGroup->clauses)
    {
        ...
        else
        {
            ...
            unordered_set<string> &setOfSynonymsToJoinOn = getSetOfSynonymsToJoinOn(toPopulate, currRes);
    ...
    try
    {
        ...
        for (int i = 0; i < groupSize; i++)
        {
            ...
            else
            {
                ...
                vector<shared_ptr<ResultTuple>> combinedRes;
                if (prevGroupHasSynonymsInResultCl)
                    cartesianProductResultTuples(currTups, tempRes, combinedRes);
                else
                    combinedRes = move(tempRes);

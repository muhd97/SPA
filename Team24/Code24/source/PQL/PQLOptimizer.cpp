...
inline string ClauseGroup::format()
{
    ...
    for (const auto &s : synonyms)
        str += s + ", ";
    str += "]";
    if (synonymsInsideResultCl)
        str += " (Has Synonyms inside ResultCl)";
    else
        str += " (No synonyms in ResultCl)";
...
vector<shared_ptr<ClauseGroup>> PQLOptimizer::getClauseGroups()
{
    ...
    if (!cgForNoSynClauses->clauses.empty())
        toReturn.emplace_back(cgForNoSynClauses);
...
void PQLOptimizer::filterTuples(vector<shared_ptr<ResultTuple>> &resultsFromClauseGroup,
                                vector<shared_ptr<ResultTuple>> &filteredResults)
{
    ...
    for (auto &ptr : resultsFromClauseGroup)
    {
        ...
        {
            if (!ptr->synonymKeyAlreadyExists(synKey))
                continue;
        ...
        if (!seenBeforeTuples.count(tempHash))
        {
            ...
            {
                if (!synonymsUsedInResultClause.count((*it).first))
                    it = underlyingMap.erase(it);
                else
                    ++it;
            }
...
inline void PQLOptimizer::DFS(OptNode *curr, unordered_map<OptNode *, vector<OptNode *>> &adjList,
                              unordered_set<OptNode *> &visited, shared_ptr<ClauseGroup> &cg)
{
...
void PQLOptimizer::BFS(OptNode *start, unordered_map<OptNode *, vector<OptNode *>> &adjList,
                       unordered_set<OptNode *> &visited, shared_ptr<ClauseGroup> &cg)
{
    queue<OptNode *> q;
    q.push(start);
...
inline void PQLOptimizer::sortSingleClauseGroup(shared_ptr<ClauseGroup> &cg)
{
    ...
    for (const auto &syn : firstClause->getAllSynonymsAsString())
    ...
    while (finalSortedOrder.size() != currGroupSize)
    {
        ...
        if (bestIdx == -1)
            throw runtime_error("Critical Error, failed to elect next best clause in group");

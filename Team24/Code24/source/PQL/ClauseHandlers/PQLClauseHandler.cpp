...
void ClauseHandler::validateStmtSyn(const string &syn, const string &relationshipType)
{
    ...
    {
        throw runtime_error("The synonym \"" + syn + "\" is not declared as a stmt! Synonyms must be stmt type for " +
                            relationshipType + ".\n");
    }
}
...
{
    if (!givenSynonymMatchesMultipleTypes(syn, {DesignEntity::PROG_LINE, DesignEntity::STMT, DesignEntity::ASSIGN}))
    {
        throw runtime_error("The synonym \"" + syn +
                            "\" is not declared as a stmt/prog_line/assign! Synonyms must be stmt type for " +
                            relationshipType + ".\n");
    }
}
...
void ClauseHandler::validateAffectsTypeInt(int stmtIdx, const string &relationshipType)
{
    ...
    {
        throw runtime_error("Statement " + to_string(stmtIdx) + " is not an assign statement in the program! " +
                            relationshipType + " must take assignment statements only.\n");
    }
}
...
void ClauseHandler::validateStmtInt(int i)
{
    ...
    {
        throw runtime_error("The provided statement index " + to_string(i) +
                            " does not exist in the SIMPLE program!\n");
    }
}

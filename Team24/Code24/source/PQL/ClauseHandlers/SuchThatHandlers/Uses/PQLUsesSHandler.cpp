...
UsesSHandler::UsesSHandler(shared_ptr<PKBPQLEvaluator> &evaluator, shared_ptr<SelectCl> &selectCl,
                           shared_ptr<UsesS> &usesSCl)
    : UsesModifiesSHandler(evaluator, selectCl, usesSCl->stmtRef, usesSCl->entRef)
{
}
...
void UsesSHandler::evaluateIntIdent(vector<shared_ptr<ResultTuple>> &toReturn)
{
    ...
    if (getEvaluator()->getUsesIntIdent(leftInt, rightIdent))
    {
        toReturn.emplace_back(getResultTuple(
            {{ResultTuple::INTEGER_PLACEHOLDER, to_string(leftInt)}, {ResultTuple::IDENT_PLACEHOLDER, rightIdent}}));
    }
}

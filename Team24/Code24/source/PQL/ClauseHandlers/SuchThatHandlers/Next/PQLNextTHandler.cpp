...
NextTHandler::NextTHandler(shared_ptr<PKBPQLEvaluator> &evaluator, shared_ptr<SelectCl> &selectCl,
                           shared_ptr<NextT> &nextTCl)
    : FollowsParentNextAffectsHandler(evaluator, selectCl, nextTCl->stmtRef1, nextTCl->stmtRef2)
{
}
...
void NextTHandler::evaluateIntInt(vector<shared_ptr<ResultTuple>> &toReturn)
{
    ...
    if (getEvaluator()->getNextTIntInt(s1, s2))
        toReturn.emplace_back(getResultTuple(
            {{ResultTuple::INTEGER_PLACEHOLDER, to_string(s1)}, {ResultTuple::INTEGER_PLACEHOLDER, to_string(s2)}}));
}

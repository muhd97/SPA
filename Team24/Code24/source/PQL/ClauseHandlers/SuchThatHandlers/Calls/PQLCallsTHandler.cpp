...
CallsTHandler::CallsTHandler(shared_ptr<PKBPQLEvaluator> &evaluator, shared_ptr<SelectCl> &selectCl,
                             shared_ptr<CallsT> &callsTCl)
    : CallsCallsTHandler(evaluator, selectCl, callsTCl->entRef1, callsTCl->entRef2)
{
}
...
void CallsTHandler::evaluateSynSyn(vector<shared_ptr<ResultTuple>> &toReturn)
{
    for (auto &p : getEvaluator()->getCallsTSynSyn())
        toReturn.emplace_back(
            getResultTuple({{getLeftArg()->getStringVal(), p.first}, {getRightArg()->getStringVal(), p.second}}));
}

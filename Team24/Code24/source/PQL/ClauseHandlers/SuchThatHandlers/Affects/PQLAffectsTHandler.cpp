...
AffectsTHandler::AffectsTHandler(shared_ptr<PKBPQLEvaluator> &evaluator, shared_ptr<SelectCl> &selectCl,
                                 shared_ptr<AffectsT> &affectsTCl)
    : FollowsParentNextAffectsHandler(evaluator, selectCl, affectsTCl->stmtRef1, affectsTCl->stmtRef2)
{
}
...
void AffectsTHandler::evaluateIntSyn(vector<shared_ptr<ResultTuple>> &toReturn)
{
    ...
    for (const auto &p : getEvaluator()->getAffects(true, leftInt).second)
    {
        if (p.first == leftInt)
...
{
    if (getEvaluator()->getAffects(getLeftArg()->getIntVal(), 0, true))
        toReturn.emplace_back(
            getResultTuple({{ResultTuple::INTEGER_PLACEHOLDER, ResultTuple::UNDERSCORE_PLACEHOLDER}}));
}

void AffectsTHandler::evaluateSynInt(vector<shared_ptr<ResultTuple>> &toReturn)
{
    int rightInt = getRightArg()->getIntVal();
    for (const auto &p : getEvaluator()->getAffects(true, rightInt).second)
    {
...
void AffectsTHandler::evaluateSynSyn(vector<shared_ptr<ResultTuple>> &toReturn)
{
    ...
    for (auto &sPair : getEvaluator()->getAffects(true, 0).second)
    {
        ...
        toReturn.emplace_back(
            getResultTuple({{leftSynonym, to_string(sPair.first)}, {rightSynonym, to_string(sPair.second)}}));
    }
}
...
{
    set<int> seen;
    for (const auto &p : getEvaluator()->getAffects(true, 0).second)
    {
...
{
    if (getEvaluator()->getAffects(0, getRightArg()->getIntVal(), true))
        toReturn.emplace_back(
            getResultTuple({{ResultTuple::UNDERSCORE_PLACEHOLDER, ResultTuple::INTEGER_PLACEHOLDER}}));
}
...
{
    if (getEvaluator()->getAffects(0, 0, true))
        toReturn.emplace_back(
            getResultTuple({{ResultTuple::UNDERSCORE_PLACEHOLDER, ResultTuple::UNDERSCORE_PLACEHOLDER}}));
}

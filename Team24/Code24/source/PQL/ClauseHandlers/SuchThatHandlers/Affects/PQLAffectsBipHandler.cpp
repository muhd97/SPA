...
AffectsBipHandler::AffectsBipHandler(shared_ptr<PKBPQLEvaluator> &evaluator, shared_ptr<SelectCl> &selectCl,
    ...
    for (const auto &p : evaluateAffectsBip())
    {
        if (p.first == leftInt && p.second == rightInt)
        {
            toReturn.emplace_back(
                getResultTuple({{ResultTuple::INTEGER_PLACEHOLDER, ResultTuple::INTEGER_PLACEHOLDER}}));
            return;
        }
    }
}

...
class WithHandler : public ClauseHandler
{
  private:
    ...
    void evaluateWithFirstArgIdentSecondArgAttr(const std::string &leftVal,
    ...
    void evaluateWithFirstArgAttrRefSecondArgInt(const std::shared_ptr<AttrRef> &leftAttrRef,
                                                 std::vector<std::shared_ptr<ResultTuple>> &toReturn);
    void evaluateWithFirstArgAttrRefSecondArgSyn(const std::shared_ptr<AttrRef> &leftAttrRef,
                                                 std::vector<std::shared_ptr<ResultTuple>> &toReturn);
    void evaluateWithFirstArgAttrRefSecondArgIdent(std::vector<std::shared_ptr<ResultTuple>> &toReturn, bool &retflag);
    void evaluateWithFirstArgAttrRefSecondArgAttr(const std::shared_ptr<AttrRef> &leftAttrRef,
                                                  std::vector<std::shared_ptr<ResultTuple>> &toReturn);
    void evaluateWithFirstArgSyn(vector<shared_ptr<ResultTuple>> &toReturn);
    void evaluateWithFirstArgSynSecondArgAttr(std::vector<std::shared_ptr<ResultTuple>> &toReturn,
                                              const std::string &leftSynonymString);
    void evaluateWithFirstArgSynSecondArgSyn(std::vector<std::shared_ptr<ResultTuple>> &toReturn,
                                             const std::string &leftSynonymString);
    void evaluateWithFirstArgSynSecondArgInt(std::vector<std::shared_ptr<ResultTuple>> &toReturn,
                                             const std::string &leftSynonymString);

  public:
    WithHandler(shared_ptr<PKBPQLEvaluator> evaluator, shared_ptr<SelectCl> &selectCl, const shared_ptr<WithCl> &withCl)
        : ClauseHandler(evaluator, move(selectCl)), withCl(move(withCl)), lhs(move(withCl->lhs)), rhs(move(withCl->rhs))
    {
    }
